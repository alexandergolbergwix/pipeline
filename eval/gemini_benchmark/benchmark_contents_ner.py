...
LABEL_SPACE: list[str] = ["WORK", "FOLIO", "WORK_AUTHOR"]
TASK = "contents_ner"
EVALUATOR_ID = "contents_ner"
CONTENTS_CHECKPOINT = "ner/contents_ner_model.pt"
PROMPT_PATH = Path(__file__).parent / "prompts" / "contents_ner_system.txt"
...
def trained_predict(pipeline: object, text: str) -> list[tuple[str, str]]:
    """Adapt NERInferencePipeline output to ``list[(span_text, type)]``."""
    spans = pipeline.process_text(text)
    out: list[tuple[str, str]] = []
    for span in spans:
        span_text = span.get("text")
        span_type = span.get("type")
        if not isinstance(span_text, str) or not isinstance(span_type, str):
            continue
        out.append((span_text, span_type))
    return out
...
def gemini_predict(
    ...
    """Call Gemini with structured output, return ``list[(text, type)]``."""
...
def few_shot_demonstrations(
    ...
    for it in items:
        gold_entities = [
            {"text": str(text), "type": str(type_)}
            for text, type_ in (it.gold or [])
        ]
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Contents NER benchmark: trained vs Gemini 0-shot vs Gemini 3-shot",
    )
    ...
    print(
        f"[contents-ner] few-shot item_ids: "
    ...
    print(
        f"[contents-ner] loading NERInferencePipeline "
    ...
    print(
        f"[contents-ner] running 3 methods on {len(items)} items…",
        file=sys.stderr,
    )
    ...
    if args.no_eval_agent:
        ...
        print("[contents-ner] running eval-agent judging…", file=sys.stderr)
        verdicts = {
    ...
    print(f"[contents-ner] wrote results to {output_dir}", file=sys.stderr)
    return 0
