...
int main(int argc, char *argv[]) {
    @autoreleasepool {
        ...
        /* Set first_run_done via a quick Python call */
        NSTask *setupTask = [[NSTask alloc] init];
        [setupTask setExecutableURL:[NSURL fileURLWithPath:pythonBin]];
        [setupTask setArguments:@[
            @"-c",
            @"from mhm_pipeline.settings.settings_manager import SettingsManager\n"
             "s=SettingsManager()\n"
             "s.first_run_done=True if not s.first_run_done else s.first_run_done"
        ]];
        [setupTask setCurrentDirectoryURL:[NSURL fileURLWithPath:pipelineDir]];
        [setupTask setEnvironment:env];
        [setupTask launchAndReturnError:nil];
