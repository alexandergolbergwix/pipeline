...
class CertaintyAnnotation:
    ...
    certainty_percentage: Optional[int] = None
...
class TextTraditionAnnotation:
    ...
    significance: Optional[str] = None  # Orthographic_variant, Lexical_variant, Syntactic_variant, Semantic_variant, Addition_variant, Omission_variant
...
class ForeignUnitAnnotation:
    ...
    addition_period: Optional[str] = None
    folio_range: Optional[str] = None
...
class AnnotationImporter:
    ...
    def __init__(self, graph: Optional[Graph] = None):
        ...
        self.graph_builder = GraphBuilder(self.uri_gen, add_epistemological_status=False, add_cataloging_view=False)
    ...
    def add_certainty(self, ann: CertaintyAnnotation):
        ...
        if ann.certainty_percentage is not None:
            self.graph.add((ms_uri, HM.certainty_percentage, 
    ...
    def add_text_tradition(self, ann: TextTraditionAnnotation):
        ...
        expression_uri = self.uri_gen.expression_uri(work_title, ann.manuscript_id) if work_title else None
    ...
    def add_canonical_reference(self, ann: CanonicalReferenceAnnotation):
        ...
        reference_string = "_".join(reference_parts) if reference_parts else f"{ann.manuscript_id}_{self._annotation_count}"
    ...
    def add_foreign_unit(self, ann: ForeignUnitAnnotation):
        ...
        self.graph.add((unit_uri, HM.is_foreign_addition, 
                       Literal(ann.is_foreign, datatype=XSD.boolean)))
            ...
            for row in reader:
                if annotation_type == AnnotationType.CERTAINTY:
                    ann = CertaintyAnnotation(
                        ...
                        certainty_percentage=int(row['certainty_percentage']) if row.get('certainty_percentage') else None,
                        note=row.get('note')
                    )
    ...
    def merge_with_ttl(self, ttl_path: Path, output_path: Optional[Path] = None) -> Path:
...
def create_sample_annotation_files(output_dir: Path):
    ...
    with open(certainty_csv, 'w', encoding='utf-8') as f:
        ...
        f.write("990000400180205171,Probable,ExpertAttribution,85,Attribution based on paleographic analysis\n")
        f.write("990000400190205171,Certain,ColophonAttribution,100,Colophon explicitly names the scribe\n")
    ...
    with open(tradition_csv, 'w', encoding='utf-8') as f:
        f.write("manuscript_id,tradition_name,work_title,variant_text,standard_text,folio,line,significance\n")
        f.write("990000400180205171,Ashkenazi Prayer Tradition,תפילה,ברוך אתה ה׳,ברוך אתה יי,12r,5,Orthographic_variant\n")
    ...
    return [certainty_csv, tradition_csv, intervention_csv, canonical_csv, relationship_csv, foreign_csv]
