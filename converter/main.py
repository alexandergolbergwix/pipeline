...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Convert MARC records to RDF/TTL format")
    parser.add_argument("--gui", "-g", action="store_true", help="Launch GUI mode")
    parser.add_argument("--input", "-i", type=Path, help="Input file (.mrc, .csv, or .tsv)")
    parser.add_argument("--output", "-o", type=Path, help="Output TTL file")
    parser.add_argument(
        "--validate",
        "-v",
        action="store_true",
    ...
    parser.add_argument("--no-validate", action="store_true", help="Skip SHACL validation")
