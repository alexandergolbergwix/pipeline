...
class MarcRecord:
    ...
    def get_control_field(self, tag: str) -> Optional[str]:
        """Get value of a control field (00X)."""
        field = self.get_field(tag)
        return field.data if field else None
    
    def get_fixed_field_value(self, tag: str, start: int, end: int) -> Optional[str]:
...
class MarcReader:
    ...
    def _parse_record(self, record: pymarc.Record) -> Optional[MarcRecord]:
        ...
        for field in record.get_fields():
            ...
            if tag.startswith('00'):
                marc_field = MarcField(
                    tag=tag,
                    data=field.data if hasattr(field, 'data') else str(field)
                )
            else:
