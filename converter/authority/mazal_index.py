...
class MazalIndex:
    ...
    def insert_authority(self, nli_id: str, entity_type: str,
    ...
    def insert_name_variant(self, normalized_name: str, nli_id: str,
                            entity_type: str, script: str = None):
    ...
    def lookup(self, name: str, entity_type: str) -> Optional[str]:
    ...
    def lookup_person(self, name: str, dates: str = None) -> Optional[str]:
    ...
    def lookup_work(self, title: str) -> Optional[str]:
        """Look up a work by title."""
        return self.lookup(title, 'work')
    
    def lookup_corporate(self, name: str) -> Optional[str]:
...
def parse_record(record_elem: ET.Element) -> Optional[dict]:
    ...
    for xml_file in xml_files:
        ...
        for nli_id, record in parse_xml_records(str(xml_file)):
            ...
            for name in record['names_heb']:
                ...
                if normalized:
                    index.insert_name_variant(
                        normalized, nli_id, record['entity_type'], 'heb'
                    )
                    total_names += 1
            ...
            for name in record['names_lat']:
                ...
                if normalized:
                    index.insert_name_variant(
                        normalized, nli_id, record['entity_type'], 'lat'
                    )
                    total_names += 1
