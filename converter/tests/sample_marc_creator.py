...
def create_sample_record() -> Record:
    ...
    record.add_field(Field(
        tag='245',
        indicators=['1', '0'],
        subfields=['a', 'גינת אגוז']
    ))
    
    record.add_field(Field(
        tag='260',
        indicators=[' ', ' '],
        subfields=['a', 'קנדיאה', 'c', '[שבט קס"ז]']
    ))
    
    record.add_field(Field(
        tag='300',
        indicators=[' ', ' '],
        subfields=['a', '151 דפים', 'c', '280 x 200 מ"מ']
    ))
    
    record.add_field(Field(
        tag='340',
        indicators=[' ', ' '],
        subfields=['a', 'קלף ונייר']
    ))
    
    record.add_field(Field(
        tag='500',
        indicators=[' ', ' '],
        subfields=['a', 'כתב ספרדי בינוני']
    ))
    ...
    record.add_field(Field(
        tag='561',
        indicators=[' ', ' '],
        subfields=['a', 'יעקב מואטי; ספריית הוותיקן']
    ))
