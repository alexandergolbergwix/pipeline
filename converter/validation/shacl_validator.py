...
class ValidationResult:
    ...
    def to_detailed_report(self, output_path: Optional[str] = None, 
        ...
        if errors:
            ...
            for msg_type, error_list in errors_by_message.items():
                ...
                for e in error_list[:50]:  # Limit to 50
                    entity_short = e.focus_node.split('#')[-1] if '#' in e.focus_node else e.focus_node.split('/')[-1]
        ...
        if warnings:
            ...
            for msg_type, warning_list in warnings_by_message.items():
                ...
                lines.append(f"<summary>Show affected entities ({min(len(warning_list), 20)} of {len(warning_list)})</summary>")
                lines.append("")
                for w in warning_list[:20]:
                    entity_short = w.focus_node.split('#')[-1] if '#' in w.focus_node else w.focus_node.split('/')[-1]
        ...
        if not errors and not warnings:
            ...
            if errors:
                ...
                for msg_type in errors_by_message.keys():
                    if "Height" in msg_type or "Width" in msg_type:
                        lines.append("- **Dimension errors**: Check source data for unrealistic measurements")
                    elif "label" in msg_type.lower():
                        lines.append("- **Missing labels**: Ensure all entities have descriptive labels")
                    else:
            ...
            if warnings:
                ...
                if any("Expression" in k for k in warnings_by_message.keys()):
                    lines.append("- **Missing Expressions**: Some manuscripts lack linked content. This often indicates incomplete bibliographic data in the source MARC records.")
                if any("Work" in k for k in warnings_by_message.keys()):
                    lines.append("- **Missing Works**: Some expressions lack work references. Use scholarly annotations to add work information.")
                lines.append("")
...
class ShaclValidator:
    ...
    def __init__(self, shapes_path: Optional[Path] = None):
    ...
    def validate_file(self, file_path: Path, 
                      format: str = 'turtle') -> ValidationResult:
    ...
    def _parse_results(self, results_graph: Graph) -> List[ValidationViolation]:
        ...
        try:
            ...
            for row in results:
                ...
                if message and any(msg in message for msg in [
    ...
    def reload_shapes(self, shapes_path: Optional[Path] = None):
...
def validate_graph(data_graph: Graph, 
                   shapes_path: Optional[Path] = None) -> ValidationResult:
