...
class SparqlExplorer(QWidget):
    ...
    def _load_file(self):
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Open TTL File", str(Path.home()), "Turtle Files (*.ttl);;All Files (*.*)"
        )
        ...
        self._load_worker.finished.connect(
            lambda g, c, t: self._on_file_loaded(Path(file_path), g, c, t)
        )
    ...
    def _on_file_loaded(self, file_path: Path, graph: Graph, triple_count: int, elapsed: float):
        ...
        self.stats_label.setText(
            f"({triple_count:,} triples, {size_mb:.1f} MB, loaded in {elapsed:.1f}s)"
        )
    ...
    def _export_results(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Export Results", str(Path.home() / "query_results.csv"), "CSV Files (*.csv)"
        )
