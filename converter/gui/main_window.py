...
from PyQt6.QtWidgets import (
    ...
    QApplication, QGroupBox, QTabWidget, QLabel
)
from PyQt6.QtCore import Qt, QThread, pyqtSignal
from PyQt6.QtGui import QAction

from .widgets import (
    FileSelector, LogViewer, TtlPreview, 
    ValidationReport, ProgressWidget
)
...
class ConversionWorker(QThread):
    ...
    def __init__(self, input_path: Path, output_path: Path, 
                 validate: bool = True, include_ontology: bool = False, parent=None):
    ...
    def run(self):
        try:
            ...
            try:
                ...
                self.log.emit(f"Found {total_records} records (format: {reader.detected_format.value})", "info")
            except:
                total_records = 0
            ...
            for record in reader.read_file():
                ...
                try:
                    ...
                    if total_records > 0:
                        ...
                        self.progress.emit(progress, f"Converted {records_processed}/{total_records} records")
                except Exception as e:
                    self.log.emit(f"Error converting record {record.control_number}: {e}", "warning")
    ...
    def _collect_statistics(self, graph, records_processed: int) -> dict:
        ...
        stats = {
            ...
            'output_path': str(self.output_path),
            'output_size_mb': self.output_path.stat().st_size / (1024 * 1024) if self.output_path.exists() else 0,
        }
        ...
        entity_counts['manuscripts'] = len(list(graph.subjects(RDF.type, LRMOO.F4_Manifestation_Singleton)))
        ...
        # Production events
        entity_counts['production_events'] = len(list(graph.subjects(RDF.type, CIDOC.E12_Production)))
        ...
        if mazal_stats.get('matcher_available', False):
            ...
            if nli_total > 0:
                ...
                if mazal_stats.get('nli_organizations', 0) > 0:
                    self.log_viewer.info(f"      🏛️ Organizations: {mazal_stats['nli_organizations']:,}")
            else:
                self.log_viewer.warning(f"   ⚠ No NLI matches found (using local URIs)")
            ...
            if 'lookups' in mazal_stats:
                ...
                self.log_viewer.info(f"   📊 Lookup stats: {hits:,}/{total_lookups:,} matches ({hit_rate:.1f}%)")
        
        # Validation summary
        if self.validate_check.isChecked():
            validation_issues = stats.get('validation_issues', 0)
            if validation_passed:
                self.log_viewer.success(f"✅ Validation: PASSED")
            else:
...
"""
        ...
        if report_path:
            view_report_btn = msg.addButton("View Validation Report", QMessageBox.ButtonRole.ActionRole)
