...
from ..config.namespaces import HM, LRMOO, CIDOC, bind_namespaces
from ..config.vocabularies import (
    LANGUAGE_CODES, DATA_FACTUALITY, 
    EPISTEMOLOGICAL_STATUSES, DATA_CATEGORIES, INTERPRETATION_METHODS,
    HIERARCHY_TYPES, PARADIGM_TYPES, CERTAINTY_LEVELS, ATTRIBUTION_SOURCES
)
...
class GraphBuilder:
    ...
    def build_graph(self, data: ExtractedData, control_number: str) -> Graph:
        ...
        if data.title:
            ...
            graph.add((main_cu_uri, RDFS.label, Literal(f"Main codicological unit of MS {control_number}", lang='en')))
        ...
        for author in data.authors:
            person_uri = self._add_person(graph, author, work_uri, 'author', related_work_title=data.title)
        ...
        for contributor in data.contributors:
            ...
            if person_uri:
                ...
                if role in ('scribe', 'copyist'):
                    ...
                    custody_uri = URIRef(f"{HM}TransferOfCustody_{control_number}_{person_local_id}")
        ...
        for index, content in enumerate(data.contents, 1):
            ...
            graph.add((cu_uri, RDFS.label, Literal(f"Codicological unit {index} of MS {control_number}", lang='en')))
            ...
            if content_record.get('folio_range'):
                graph.add((cu_uri, HM.has_folio_range, Literal(content_record['folio_range'], datatype=XSD.string)))
        ...
        if not structural_cu_uris:
            ...
            graph.add((default_cu_uri, RDFS.label, Literal(f"Codicological unit 1 of MS {control_number}", lang='en')))
        ...
        for idx in range(pu_count):
            ...
            graph.add((pu_uri, RDFS.label, Literal(f"Paleographical unit {idx + 1} of MS {control_number}", lang='en')))
            ...
            if idx < len(scribe_entity_uris):
                ...
                graph.add((scribe_uri, RDFS.label, Literal(f"Unknown scribe {idx + 1} (MS {control_number})", lang='en')))
        ...
        if work_expression_pairs:
            phil_view_uri = self.add_philological_view(graph, ms_uri, control_number, is_primary=False)
    ...
    def _add_manuscript(self, graph: Graph, ms_uri: URIRef, data: ExtractedData,
        ...
        graph.add((ms_uri, HM.external_identifier_nli, 
                   Literal(control_number, datatype=XSD.string)))
        
        label = data.title or f"MS {control_number}"
        graph.add((ms_uri, RDFS.label, Literal(label, lang='he')))
        
        if data.extent:
            graph.add((ms_uri, HM.has_number_of_folios,
                       Literal(int(data.extent), datatype=XSD.integer)))
        
        if data.height_mm:
            graph.add((ms_uri, HM.has_height_mm,
                       Literal(int(data.height_mm), datatype=XSD.integer)))
        
        if data.width_mm:
            graph.add((ms_uri, HM.has_width_mm,
                       Literal(int(data.width_mm), datatype=XSD.integer)))
        ...
        if data.digital_url:
            graph.add((ms_uri, HM.has_digital_representation_url, 
                       Literal(data.digital_url, datatype=XSD.anyURI)))
        
        if data.provenance:
            graph.add((ms_uri, HM.ownership_history, 
                       Literal(data.provenance, datatype=XSD.string)))
        
        for note in data.notes:
            graph.add((ms_uri, RDFS.comment, Literal(note, lang='he')))
    
    def _add_work(self, graph: Graph, data: ExtractedData, 
                  author_name: Optional[str] = None) -> URIRef:
    ...
    def _add_expression(self, graph: Graph, work_uri: URIRef, ms_uri: URIRef,
                        data: ExtractedData, control_number: str) -> URIRef:
        ...
        expr_label = f"{data.title} (in MS {control_number})" if data.title else f"Expression in MS {control_number}"
    ...
    def _add_production_event(self, graph: Graph, ms_uri: URIRef, 
        ...
        if data.dates:
            ...
            if 'date_start' in data.dates:
                graph.add((time_uri, HM.earliest_possible_date, 
            ...
            if 'date_end' in data.dates:
                graph.add((time_uri, HM.latest_possible_date, 
            ...
            if 'original_string' in data.dates:
                graph.add((time_uri, HM.date_original_string, 
                           Literal(data.dates['original_string'])))
            graph.add((prod_uri, HM.has_production_date_certain,
    ...
    def _add_person(self, graph: Graph, person_data: Dict[str, Any],
                    related_uri: Optional[URIRef], role: str,
                    related_work_title: Optional[str] = None) -> Optional[URIRef]:
        ...
        graph.add((person_uri, RDFS.label, 
                   Literal(person_data['name'], lang='he')))
        
        if 'birth_year' in person_data:
            graph.add((person_uri, CIDOC.P82a_begin_of_the_begin, 
                       Literal(person_data['birth_year'], datatype=XSD.integer)))
        
        if 'death_year' in person_data:
            graph.add((person_uri, CIDOC.P82b_end_of_the_end, 
        ...
        if auth_data.get('external_uri_nli'):
            graph.add((person_uri, HM.external_uri_nli,
                       Literal(auth_data['external_uri_nli'], datatype=XSD.anyURI)))
        if auth_data.get('viaf_id'):
            graph.add((person_uri, HM.viaf_id,
                       Literal(auth_data['viaf_id'], datatype=XSD.string)))
        if auth_data.get('wikidata_id'):
            graph.add((person_uri, HM.wikidata_id,
                       Literal(auth_data['wikidata_id'], datatype=XSD.string)))
        ...
        if related_uri and role == 'author' and related_work_title:
            creation_uri = self.uri_gen.work_creation_event_uri(related_work_title, person_data['name'])
    ...
    def _add_content_work(self, graph: Graph, content: Dict[str, Any],
                          ms_uri: URIRef, control_number: str) -> Optional[Dict[str, Any]]:
        ...
        graph.add((expression_uri, RDF.type, LRMOO.F2_Expression))
        graph.add((expression_uri, RDFS.label, 
                   Literal(f"{content['title']} (in MS {control_number})", lang='he')))
        graph.add((expression_uri, LRMOO.R3_is_realised_in, work_uri))
        
        if content.get('folio_range'):
            graph.add((expression_uri, HM.has_folio_range, 
        ...
        if content.get('sequence') is not None:
            ...
            graph.add((position_bnode, HM.anthology_order,
    ...
    def _add_multi_volume_set(self, graph: Graph, ms_uri: URIRef,
                              data: ExtractedData, control_number: str):
    ...
    def _add_anthology_structure(self, graph: Graph, ms_uri: URIRef,
                                 control_number: str, works_count: int):
        ...
        graph.add((anthology_uri, RDFS.label,
                   Literal(f"Anthology structure of MS {control_number}", lang='en')))
        graph.add((ms_uri, HM.has_anthology_structure, anthology_uri))
        graph.add((anthology_uri, HM.number_of_works,
                   Literal(works_count, datatype=XSD.integer)))
    
    def _add_subject(self, graph: Graph, subject: Dict[str, Any],
                     ms_uri: URIRef, work_uri: Optional[URIRef]):
        ...
        if subject.get('authority_id'):
            graph.add((subject_uri, HM.external_uri_nli, 
    ...
    def _add_catalog_reference(self, graph: Graph, ref: Dict[str, str],
                               ms_uri: URIRef):
    ...
    def _add_colophon(self, graph: Graph, ms_uri: URIRef, 
                      colophon_text: str, control_number: str):
        ...
        graph.add((colophon_uri, HM.colophon_text, 
                   Literal(colophon_text, lang='he')))
        graph.add((colophon_uri, HM.has_colophon_text,
                   Literal(colophon_text, lang='he')))
        
        graph.add((ms_uri, HM.has_colophon, colophon_uri))
    
    def _add_binding(self, graph: Graph, ms_uri: URIRef,
                     binding_info: str, control_number: str):
        ...
        graph.add((binding_uri, RDFS.comment, 
    ...
    def _add_cataloging_view(self, graph: Graph, ms_uri: URIRef, 
                              work_uri: URIRef, expression_uri: Optional[URIRef],
                              control_number: str):
        ...
        graph.add((cat_view_uri, RDFS.label, 
        ...
        graph.add((cat_view_uri, HM.is_primary_paradigm, 
                   Literal(True, datatype=XSD.boolean)))
        
        # Add paradigm note explaining this is catalog-derived data
        graph.add((cat_view_uri, HM.paradigm_note,
                   Literal("Data derived from NLI catalog using bibliographic Work-Expression model", 
                           lang='en')))
    
    def _add_epistemological_metadata(self, graph: Graph, ms_uri: URIRef,
                                       control_number: str):
        ...
        graph.add((evidence_chain_uri, RDFS.label,
        ...
        graph.add((catalog_step_uri, HM.reasoning_text,
                   Literal("Data imported from National Library of Israel MARC catalog", 
                           lang='en')))
        ...
        graph.add((ms_uri, HM.is_factual, 
                   Literal(False, datatype=XSD.boolean)))  # Mixed data - conservative
    
    def _add_certainty(self, graph: Graph, subject_uri: URIRef, 
                       certainty_level: str, certainty_note: Optional[str] = None):
        ...
        if certainty_note:
            graph.add((subject_uri, HM.certainty_note,
                       Literal(certainty_note, datatype=XSD.string)))
    
    def _add_attribution_source(self, graph: Graph, subject_uri: URIRef,
                                 source_type: str, attributed_by: Optional[str] = None):
    ...
    def add_codicological_hierarchy(self, graph: Graph, ms_uri: URIRef,
                                     control_number: str, 
                                     hierarchy_type: str = "SimpleHierarchy",
                                     max_depth: int = 1) -> URIRef:
        ...
        graph.add((hierarchy_uri, HM.max_nesting_depth, 
                   Literal(max_depth, datatype=XSD.integer)))
        
        return hierarchy_uri
    
    def add_codicological_unit(self, graph: Graph, ms_uri: URIRef,
                                control_number: str, sequence: int,
                                is_atomic: bool = True,
                                nesting_level: int = 0,
                                parent_uri: Optional[URIRef] = None,
                                folio_range: Optional[str] = None,
                                unit_status: str = "CoreUnit_status") -> URIRef:
        ...
        if is_atomic:
            ...
            graph.add((cu_uri, HM.is_atomic_unit, 
                       Literal(True, datatype=XSD.boolean)))
        else:
        ...
        graph.add((cu_uri, RDF.type, HM.Codicological_Unit))
        graph.add((cu_uri, HM.nesting_level, 
                   Literal(nesting_level, datatype=XSD.integer)))
        graph.add((cu_uri, HM.unit_sequence, 
                   Literal(sequence, datatype=XSD.integer)))
        ...
        if folio_range:
            graph.add((cu_uri, HM.has_folio_range, 
                       Literal(folio_range, datatype=XSD.string)))
    ...
    def add_philological_view(self, graph: Graph, ms_uri: URIRef,
                               control_number: str,
                               is_primary: bool = False) -> URIRef:
        ...
        graph.add((phil_view_uri, HM.is_primary_paradigm, 
                   Literal(is_primary, datatype=XSD.boolean)))
        graph.add((phil_view_uri, HM.paradigm_note,
                   Literal("New Philology view: manuscript as unique cultural event", 
                           lang='en')))
        
        return phil_view_uri
    
    def add_text_tradition(self, graph: Graph, tradition_name: str,
        ...
        graph.add((tradition_uri, HM.tradition_name, 
                   Literal(tradition_name, datatype=XSD.string)))
        
        if description:
            graph.add((tradition_uri, HM.tradition_description,
                       Literal(description, datatype=XSD.string)))
        
        return tradition_uri
    
    def add_transmission_witness(self, graph: Graph, ms_uri: URIRef,
                                  tradition_uri: URIRef,
                                  control_number: str,
                                  work_title: str,
                                  expression_uri: Optional[URIRef] = None,
                                  philological_view_uri: Optional[URIRef] = None) -> URIRef:
        ...
        graph.add((witness_uri, RDFS.label, 
        ...
        if philological_view_uri is None:
            philological_view_uri = self.add_philological_view(graph, ms_uri, control_number, is_primary=False)
    ...
    def add_paradigm_bridge(self, graph: Graph, work_uri: URIRef,
                             tradition_uri: URIRef,
                             work_title: str,
                             tradition_name: str,
                             justification: Optional[str] = None) -> URIRef:
        ...
        if justification:
            graph.add((bridge_uri, HM.paradigm_justification,
                       Literal(justification, datatype=XSD.string)))
        
        return bridge_uri
    
    def add_textual_variant(self, graph: Graph, ms_uri: URIRef,
                             control_number: str,
                             location: str,
                             variant_text: str,
                             standard_text: Optional[str] = None,
                             significance: str = "Lexical_variant",
                             expression_uri: Optional[URIRef] = None) -> URIRef:
        ...
        graph.add((variant_uri, HM.variant_text, 
                   Literal(variant_text, lang='he')))
        
        if standard_text:
            graph.add((variant_uri, HM.standard_text,
                       Literal(standard_text, lang='he')))
    ...
    def add_scribal_intervention(self, graph: Graph, ms_uri: URIRef,
                                   control_number: str,
                                   sequence: int,
                                   intervention_type: str,
                                   location: Optional[str] = None,
                                   description: Optional[str] = None,
                                   by_scribe: Optional[str] = None) -> URIRef:
        ...
        if description:
            graph.add((intervention_uri, HM.intervention_description,
                       Literal(description, datatype=XSD.string)))
    ...
    def add_canonical_reference(self, graph: Graph, expression_uri: URIRef,
                                 hierarchy_type: str,
                                 canonical_start: str,
                                 canonical_end: Optional[str] = None) -> URIRef:
        ...
        """
        ref_string = canonical_start if not canonical_end else f"{canonical_start}_to_{canonical_end}"
        ...
        graph.add((ref_uri, HM.canonical_start, 
                   Literal(canonical_start, datatype=XSD.string)))
        
        if canonical_end:
            graph.add((ref_uri, HM.canonical_end,
                       Literal(canonical_end, datatype=XSD.string)))
        
        return ref_uri
    
    def add_detailed_evidence_chain(self, graph: Graph, subject_uri: URIRef,
                                     control_number: str,
                                     data_field: str,
                                     epistemological_status: str,
                                     interpretation_method: Optional[str] = None,
                                     evidence_strength: Optional[float] = None,
                                     reasoning_text: Optional[str] = None) -> URIRef:
        ...
        graph.add((subject_uri, HM.is_factual, 
                   Literal(is_factual, datatype=XSD.boolean)))
        ...
        if evidence_strength is not None:
            graph.add((chain_uri, HM.evidence_strength,
                       Literal(evidence_strength, datatype=XSD.decimal)))
        
        if reasoning_text:
            graph.add((chain_uri, HM.reasoning_text,
                       Literal(reasoning_text, datatype=XSD.string)))
