...
\item \textbf{Named Entity Recognition (NER) System} --- a three-model deep-learning pipeline: (1)~a DictaBERT-based role-aware person NER model (80.4\% strict span+role F1; 86.8\% name-only F1; 92.6\% role accuracy when the name span is matched, measured on the representative 100-record held-out benchmark sample) that extracts person names and roles (AUTHOR, TRANSCRIBER, OWNER, CENSOR, TRANSLATOR, COMMENTATOR) directly from BIO role labels in note fields (MARC 500, 520, 545, 546, 561, 957); (2)~a provenance NER model (95.91\% F1, v2 multi-entity augmented) that extracts OWNER, DATE, and COLLECTION entities from MARC 561; and (3)~a contents NER model (99.99\% F1) that extracts WORK, FOLIO, and WORK\_AUTHOR entities from MARC 505.
...
The pipeline's canonical scholarly output is the validated Stage~4/5 HMO Turtle graph (\code{output.ttl}). The final stage projects that graph to Wikidata, making a safe public subset of Hebrew manuscript metadata accessible as part of the global Linked Open Data (LOD) ecosystem. An optional HMO Wikibase export can also deploy the full scholarly model in a browsable and editable interface, but it is a deployment layer rather than a replacement for the ontology-native TTL graph.
...
The pipeline's canonical output is the validated HMO RDF graph. That graph can be published in three complementary ways: retained as \code{output.ttl} for ontology-native research and preservation, exported optionally to an HMO Wikibase instance for browsable/editable scholarly curation, and projected conservatively to Wikidata so metadata are discoverable and queryable within the global knowledge graph alongside data from museums, archives, and libraries worldwide.
...
\item[Stage 2: NER Extraction]
  Three NER models run sequentially: (1)~the person NER model (\code{JointNERPipeline}) extracts person name spans from note fields (500, 520, 545, 546, 561, 957) with roles encoded directly in the token labels; (2)~the provenance NER model (\code{NERInferencePipeline}) extracts OWNER, DATE, and COLLECTION entities from MARC 561 provenance text; (3)~the contents NER model extracts WORK, FOLIO, and WORK\_AUTHOR entities from MARC 505. All entities are merged into a single list with a \code{source} discriminator (\code{person\_ner}, \code{provenance\_ner}, \code{contents\_ner}).

\item[Stage 3: Authority Resolution]
  Extracted entity names (persons, places, works) are matched against multiple authority sources: the NLI Mazal authority file (local SQLite database with fuzzy matching), VIAF (public SRU API, persons), and KIMA --- an open, attestation-based database of 48,128 Hebrew historical place names, 100\% Wikidata-linked, queried via a local SQLite index. Matched entities receive stable global identifiers. The stage writes \code{authority\_enriched.json}, which is both the input to Stage~4 and an \emph{evaluation target}: the sibling eval-agent's \code{authority} evaluator reads this file and judges, per match, whether the resolved authority record is the correct real-world entity for the catalogued name (triggered by a ``\emph{Verify with AI agent}'' action on the Authority panel). Curator approval of a match is recorded in the shared \code{approvals.json} sidecar (see \S\ref{sec:inventory}), keyed canonically by control number, group, role/type, and normalised text, so the decision is shared across the manual authority editor and the AI-verdict review surface.

\item[Stage 4: RDF Graph Construction]
  The \code{GraphBuilder} assembles an RDF graph from the enriched extracted data. When curator-reviewed authority data are available, Stage~4 uses \code{authority\_enriched\_reviewed.json}; otherwise it falls back to \code{authority\_enriched.json}. The later \code{*\_wikidata\_verified.json} files are Wikidata Studio review state and are never used as manuscript-source input for RDF construction. The \code{UriGenerator} creates stable URIs for each entity. The builder implements the HMO three-layer architecture: creating F4\_Manifestation\_Singleton instances, Work-Expression pairs, BU-CU-PU composition structures, production and transfer events, text traditions, and epistemological annotations.
  Stage~4 emits both \code{lrmoo:R3\_is\_realised\_in} and its inverse \code{lrmoo:R3i\_realises} for Work--Expression links, creates an explicit unidentified Work--Expression pair when the MARC record lacks a usable title or contents entry, attaches \code{hm:canonical\_hierarchy} to generated canonical references, and supplies explicit \code{hm:TextLocation} nodes for detected scribal interventions when the source record does not provide a folio-level location. \code{hm:MultiVolumeSet} is emitted only when at least two volume members can be asserted, preserving the ontology's SHACL cardinality requirement. Each RDF build writes \filepath{ontology\_coverage.json}, reporting how many declared \code{hm:} classes and properties appear in the TTL; CI ratchets this metric on the committed golden fixture at \filepath{data/fixtures/ontology\_golden/} (distinct from Wikidata projection coverage).
  The resulting \code{output.ttl} is the canonical full HMO scholarly graph. Downstream Wikibase or Wikidata exports are projections/deployments of this graph and must not become the source of record for HMO semantics.

\item[Stage 5: SHACL Validation]
  The generated graph is validated against \code{shacl-shapes.ttl} using PyShacl. Violations are reported with severity levels (Violation, Warning, Info) and specific focus nodes, enabling quality assurance before publication. Optional descriptive details such as folio ranges on expressions and mentioned scribes in colophons are type-checked when present, but are not required for every record because MARC source data do not consistently expose them.

\item[Stage 6: Wikidata Upload]
  The validated HMO RDF graph is projected into Wikidata statements and uploaded via the Wikidata API and/or QuickStatements. The RDF graph remains the full scholarly representation, while Wikidata is the richest safe public projection. A projection coverage report records which HMO structures were mapped directly, summarised, or intentionally kept ontology-native. For backward compatibility the legacy JSON-based builder remains available, but the preferred Stage~6 input is the Stage~4/5 \code{output.ttl} generated from reviewed authority data.

  English labels for work items and Hebrew-only person aliases go through a five-tier transliteration waterfall (Rule~46 in \filepath{CLAUDE.md}): a curated override dictionary for famous figures (\textit{Maimonides}, \textit{Rashi}, \textit{Ibn Ezra}, \textit{Joseph Karo}, …), then NLI ALA-LC romanizations read from the MARC source record (fields 246, 880), then a cached Wikidata SPARQL reverse-lookup that finds the community-consensus English label when one exists, then ML-based transliteration via TaatikNet\footnote{\code{malper/taatiknet} \textemdash{} a ByT5-small seq2seq Hebrew\,$\leftrightarrow$\,Latin transliterator by Morris Alper trained on $\sim$15k Wiktionary pairs.}, and finally a deterministic ALA-LC consonantal fallback. Work items with Hebrew-only titles get a compound label of the form \code{"\textless translit\textgreater{} (NLI \textless control\_number\textgreater)"}, pairing a readable transliteration with an unambiguous identifier back to the source record. The labels and TaatikNet inference are fully offline-capable; the model ($\sim$1.1\,GB) is bundled identically on macOS and Windows, with cross-platform parity enforced by an integration test class.
\end{description}

\paragraph{Optional HMO Wikibase deployment.}
In addition to the six-stage MARC-to-Wikidata flow, the desktop application exposes an optional \emph{HMO Wikibase} workbench fed by the same canonical \code{output.ttl}. This workbench builds offline project-Wikibase drafts (\code{wikibase\_entities.json}, \code{wikibase\_quickstatements.txt}, and \code{wikibase\_export\_report.json}) and can test the read-only API endpoint of the project Wikibase instance. It performs no upload in its initial form. Its purpose is to prepare a project-controlled Wikibase representation that can preserve HMO classes and relationships that Wikidata cannot natively express. The current export audit confirms one draft entity per typed HMO node and preservation of all RDF-derived non-\code{rdf:type}/non-\code{rdfs:label} statements; \code{rdf:type} and \code{rdfs:label} are represented as entity class metadata and multilingual labels. Nodes with multiple RDF classes currently expose one preferred \code{class\_uri}; a future \code{class\_uris} field is planned so secondary type assertions are also explicit in the JSON draft.
...
\item \code{ner.inference\_pipeline.JointNERPipeline} --- person NER with role-aware BIO labels
...
\subsection{Deployment Modes for Stage~2 Inference}
\label{sec:deployment-modes}

The four Stage~2 model checkpoints (joint Person NER, Provenance NER, Contents NER, Genre classifier) can be hosted in three different ways. The application-level code that consumes them is identical; only the inference \emph{host} differs.

\begin{itemize}
\item \textbf{Desktop (in-process torch).} The macOS DMG and Windows installer bundle the four \code{.pt} checkpoints (~2.5\,GB total) alongside a DictaBERT base directory. \code{JointNERPipeline} and \code{NERInferencePipeline} call \code{torch.load} directly on those bundled files and use the MPS$\rightarrow$CUDA$\rightarrow$CPU device chain. This is the canonical research deployment: offline, no per-call cost, ~50\,ms per record on Apple Silicon. The cross-platform parity contract (CLAUDE.md Rule~46) forbids dropping a model from either installer.

\item \textbf{Collaborative web port (FastAPI backend).} A sibling project at \code{mhm-pipeline-web} exposes the same pipeline as a FastAPI service with a React/TypeScript browser frontend. The backend selects between three Stage~2 inference backends via an \code{EXTRACTION\_MODE} environment variable (\code{local}, \code{hf-api}, or \code{modal}). The remaining stages (3--6) are byte-identical to the desktop tree, re-mirrored through a \code{sync-from-desktop} command.

\item \textbf{Pay-per-call inference backend (Modal).} For web deployments that cannot bundle ~2.5\,GB of weights into the application slug, all four checkpoints are also published as public Hugging Face Hub repositories (\code{alexgoldberg/hebrew-manuscript-joint-ner-v2}, \code{-provenance-ner}, \code{-contents-ner}, \code{-genre-classifier}) and packaged in a single serverless Modal container (\code{mhm-pipeline-web/modal/modal\_app.py}). The container loads all four models at warm-up, shares the DictaBERT base across the three derived NER models, and exposes one \code{POST /extract} endpoint that returns Person, Provenance, Contents, and Genre output in one round-trip per record. Cold start: ~30--60\,s for the four-model load; warm latency: ~300\,ms per record on a CPU-2 container. Cost is approximately \$0.00006 per record, well under \$1 for a 10{,}000-record corpus; Modal's free-tier credit covers typical research workloads entirely. The Modal container vendors the same desktop \code{JointNERPipeline} / \code{NERInferencePipeline} / \code{GenreClassifier} loader code via \code{image.add\_local\_dir(..., copy=True)}; weights are baked into the image at build time so cold start is just \code{torch.load}, not a network fetch.
\end{itemize}

A shared cross-user inference cache (Postgres JSONB table \code{inference\_cache}, content-addressed by SHA-256 of canonical query JSON) routes every external NER, classifier, and authority call through a single lookup function. The first team member to resolve a candidate populates the cache; everyone else warm-hits in milliseconds regardless of which inference host actually served the original call. The desktop application uses on-disk verdict caches per matcher; the web port uses the same logical contract via the shared DB table.
...
\hline
Provenance events & 541, 583 & provenance\_events (typed, dated, place-bearing custody events: acquisition place from 541 \$b, conservation/exhibition site from 583 \$j; Rule 60) \\
\hline
...
\hline
541 & Immediate Source of Acquisition & Source (\$a), address (\$b), method (\$c), date (\$d). \$b yields an \emph{acquisition} provenance event geocoded via KIMA/gazetteer in Stage 3 (Rule 60) \\
\hline
561 & Ownership/Custody History & Provenance narrative; NER extraction target \\
\hline
583 & Action Note & Action (\$a), site (\$j), jurisdiction (\$h), date (\$c). \$j yields a \emph{conservation}/\emph{exhibition} provenance event geocoded via KIMA/gazetteer in Stage 3 (Rule 60) \\
\hline
600 & Subject (Person) & Subject persons; routed through Stage 3 authority matcher with \code{role="subject"} (Rule 49) \\
\hline
610 & Subject (Corporate) & Subject organisations; routed through Stage 3 authority matcher (Rule 49) \\
\hline
650 & Subject (Topical) & Subject headings \\
\hline
651 & Subject (Geographic) & Geographic subjects; routed through KIMA in Stage 3 (Rule 49) \\
\hline
700 & Added Entry (Person) & Additional persons; subfields \$a, \$d, \$e \\
\hline
710 & Added Entry (Corporate) & Added corporate names; subfields \$a, \$4 \\
\hline
711 & Added Entry (Meeting) & Added meeting names; subfields \$a, \$4 \\
\hline
752 & Hierarchical Place Name & Country/state/city hierarchy; most-specific component routed through KIMA (Rule 49) \\
\hline
800 & Series Added Entry (Person) & Series person entries; tagged \code{field="800"} for downstream authority matching (Rule 49) \\
\hline
810 & Series Added Entry (Corporate) & Series corporate entries; tagged \code{field="810"} (Rule 49) \\
\hline
811 & Series Added Entry (Meeting) & Series meeting entries; tagged \code{field="811"} (Rule 49) \\
\hline
...
\node[note, above=0.1cm of person] {80.3\% span+role F1};
\node[note, right=0.1cm of prov] {95.91\% F1};
\node[note, right=0.1cm of cont] {99.99\% F1};

\end{tikzpicture}
\caption{Three-model NER architecture. Person NER uses role-aware BIO token labels; provenance v2 and contents models each load independent DictaBERT instances.}
\label{fig:ner-architecture}
\end{figure}

\subsubsection{Model 1: Person NER (JointNERPipeline)}

The person NER model is based on DictaBERT, fine-tuned for role-aware token classification with BIO labels such as B-AUTHOR, I-AUTHOR, B-OWNER, and I-OWNER. It is trained on an isolated v3 dataset combining the earlier multi-entity person corpus with relator-derived role examples, keeping the legacy bundle name \code{hebrew-manuscript-joint-ner-v2} so application code can continue to instantiate \code{JointNERPipeline}.

Key characteristics:
\begin{itemize}
\item Handles 1--10 persons per text segment
\item Multi-entity and relator-derived role examples improve direct role supervision
\item F1 score on the representative 100-record held-out benchmark sample: 80.4\% strict span+role; 86.8\% name-only; 92.6\% role accuracy among matched names (local model directory: \code{models/hebrew-manuscript-joint-ner-v2})
\end{itemize}

\subsubsection{Role Classification: Role-Aware BIO Labels}

The current person NER checkpoint predicts the role jointly with each name span, so a span labelled B-TRANSCRIBER/I-TRANSCRIBER is emitted directly as a TRANSCRIBER entity. \code{JointNERPipeline} still retains the old keyword fallback for legacy checkpoints, but the bundled application model uses neural role-aware BIO labels for the six production roles:
...
Training: 12,100 samples (28.4\% multi-entity augmented) from 123K MARC records; 5-fold cross-validation with DictaBERT + NER head; \code{max\_length=128}, \code{batch\_size=32}, \code{lr=3e-5}. F1 score: 95.91\% (v2, multi-entity augmented; v1 single-entity was 93.96\%).
...
Each NER model loads its own DictaBERT instance independently ($\sim$2.1\,GB total for 3 models). Shared-base loading via \code{NERInferencePipeline.from\_shared\_base()} was found to corrupt BERT weights when full checkpoints (containing \code{bert.*} keys) are loaded with \code{load\_state\_dict(strict=False)} --- see Section~5.2 of \filepath{ner/NER\_TRAINING\_REPORT.md}. Model paths resolve in order: explicit kwarg $\to$ environment variable (\code{MHM\_BUNDLED\_PROVENANCE\_MODEL} / \code{MHM\_BUNDLED\_CONTENTS\_MODEL}) $\to$ repo-relative fallback (\filepath{ner/provenance\_ner\_model.pt}).
...
Person NER & HalleluBERT (single) & 86.56\% name-only F1 & 8,186 single-person \\
\textbf{Person NER v3} & \textbf{DictaBERT role-aware BIO} & \textbf{80.4\% span+role F1; 86.8\% name-only F1} & \textbf{6,823 train / 911 val / 904 test; representative 100-record benchmark} \\
\midrule
Role & BIO token labels & 92.7\% given matched name & Six production roles \\
\midrule
\textbf{Provenance NER v2} & \textbf{DictaBERT + NER head} & \textbf{95.91\% F1} & \textbf{12.1K (28.4\% multi-entity)} \\
\textbf{Contents NER} & \textbf{DictaBERT + NER head} & \textbf{99.99\% F1} & \textbf{10K from 123K MARC} \\
\midrule
\textbf{Colophon~ML (MARC 500)} & \textbf{DictaBERT multi-label (2 heads)} & \textbf{F1 $\approx$~96\%} & \textbf{Sentence-split MARC 500 notes} \\
\textbf{Genre~ML (P136 fallback)} & \textbf{DictaBERT multi-label (8 classes + NOTA)} & \textbf{micro-F1 0.918} & \textbf{25.4K Ktiv records (keyword-gated)} \\
\bottomrule
\end{tabular}
\caption{Five ML models bundled with the pipeline: three token-level NER models (Stage~2), a sentence-level MARC~500 classifier that routes colophon/provenance sentences, and a genre classifier that fills P136 at Stage~3 for records without MARC~655.}
\label{tab:ner-models}
\end{table}

\subsubsection{Model 4: Colophon~ML (MARC~500 sentence classifier)}

A single DictaBERT encoder with two independent sigmoid heads classifies each sentence in MARC~500 general notes as \textsc{Colophon} and/or \textsc{Provenance} (multi-label: a sentence may be both). Colophon sentences feed into P1684 (inscription); Provenance sentences are re-routed into the Provenance NER pipeline at inference time, extending owner/date/collection coverage to MARC~500 in addition to MARC~561. The model lives as a lazy-loaded singleton in \code{src/mhm\_pipeline/controller/workers.py}; when the weight file is absent the pipeline degrades gracefully to a 4-keyword Hebrew rule.

\subsubsection{Model 5: Genre~ML (P136 classifier, Stage~3)}

A distant-supervision classifier trained on 25,421 Ktiv records whose MARC~500 notes contain at least one whole-token match against the \texttt{GENRE\_KEYWORDS} dictionary, plus 1{,}629 NOTA (``none of the above'') abstention examples. Eight high-confidence genre classes are retained after filtering classes with fewer than 100 training examples. The architecture is DictaBERT warm-started from the Provenance NER checkpoint, with the top 2 of 12 encoder layers fine-tuned under differential learning rates, focal loss ($\gamma=2$), and per-fold threshold tuning. At inference, the model runs only when a record has no MARC~655 heading; inferred genres are written as P136 claims qualified with P1480 (``presumably'') and a P887 reference pointing to Q2539 (machine learning). The classifier lives in \code{converter/wikidata/item\_builder.py} as a lazy-loaded singleton. Expected P136 coverage impact on Ktiv: 69\%~$\to$~$\approx$85\%.
...
\textbf{Provenance NER:} Three extraction strategies from MARC 561: structured cross-referencing against authority fields, rule-based pattern matching for ownership phrases, and Latin censor name identification. Produced 10K curated samples from 123K records, augmented to 12.1K samples with 28.4\% multi-entity concatenations (v2).
...
Provenance NER v2 (multi-entity) & 9,680 & 1,210 & 1,210 \\
...
# Model 1: Person NER (role-aware BIO labels)
person_pipeline = JointNERPipeline(model_path="models/hebrew-manuscript-joint-ner-v2")
...
\subsection{VIAF Integration}

The Virtual International Authority File (VIAF) aggregates authority records from national libraries worldwide. VIAF integration uses the VIAF SRU/JSON API in two phases:

\begin{enumerate}
\item \textbf{Name matching} --- searches personal-name headings via \code{https://viaf.org/viaf/search} (SRU with CQL, \code{Accept: application/json} header). Rate-limited to 2 requests/second. Returns VIAF cluster URIs for matched entities.
\item \textbf{Cluster harvesting} --- fetches \code{https://viaf.org/viaf/\{id\}} with \code{Accept: application/json} header (the \code{/viaf.json} endpoint was removed) and extracts cross-referenced authority identifiers from the \code{ns1:VIAFCluster} JSON response:
  \begin{description}[leftmargin=1.5cm]
  \item[P227] GND ID (Deutsche Nationalbibliothek)
  \item[P244] LCCN (Library of Congress)
  \item[P268] BnF ID (Biblioth\`{e}que nationale de France)
  \item[P213] ISNI (International Standard Name Identifier)
  \end{description}
  Results are cached per-instance. Expected yield: 2--4 additional external identifiers per person with a VIAF match ($\sim$35\% of persons).
\end{enumerate}

Implementation: \code{VIAFMatcher} class in \filepath{converter/authority/viaf\_matcher.py} with \code{match\_person()} and \code{get\_cluster\_identifiers()} methods. Enabled by default in the authority stage.
...
\textbf{Persons (NER-extracted \emph{or} from MARC name fields 100/110/111/700/710/711/600/610/800/810/811, see Rule 49):}
\begin{enumerate}
\item \textbf{Local (Mazal):} Query the NLI authority file first (fastest, domain-specific).
\item \textbf{VIAF:} If no Mazal match, query VIAF via the public SRU API (no API key required). VIAF cluster harvesting also surfaces birth/death years (\code{ns1:birthDate}~/~\code{ns1:deathDate}).
\item \textbf{Wikidata SPARQL:} Triangulation by VIAF~ID (P214) or NLI~J9U~ID (P8189). If no identifier resolves, fall back to a Hebrew label search. When a candidate QID resolves, query \code{wdt:P569}~/~\code{wdt:P570} via \code{find\_dates\_by\_qid} so birth/death years feed the Stage~3 date-conflict guard for Wikidata-only candidates as well (Rule 49).
\item \textbf{Date-conflict guard} (\code{stage3\_guards.evaluate\_date\_conflict}, 5-year birth buffer, 80-year death-after-life threshold). Roles in \code{PHYSICAL\_PRODUCTION\_ROLES} (scribe, transcriber, copyist) get the death check; \code{TEXTUAL\_AUTHORSHIP\_ROLES} (author, translator, commentator, editor) get only the birth check; \code{role="subject"} (MARC 600/610) gets only the birth check because a manuscript can be ABOUT someone who died centuries before its production.
\item \textbf{Unresolved:} Entity flagged with name only; manual review in the Authority editor (with structured-breakdown tooltip on the confidence column, Rule 49).
\end{enumerate}
...
\textbf{Curator review (Wikidata Studio → Authority panel).} Each match surfaced by the pipeline is reviewed by a human curator before it feeds the downstream RDF build. A dedicated \emph{``Review \& Edit Authority Matches''} dialog exposes every proposed match with a per-row \textbf{🧬 Compare} button that opens a side-by-side biodata comparison between the MARC record and the matched authority (Mazal / VIAF / KIMA). The comparison covers dates (birth / death / range), places (birth-place / death-place / country / coordinates), names in every known script (Hebrew / Latin / English / Arabic), and occupations; each field is pill-coloured \emph{matched} / \emph{differs} / \emph{only-in-MARC} / \emph{only-in-authority} after Unicode NFKC normalisation. A Raw tab shows both sides as pretty-printed JSON with cross-key value matches highlighted in blue. Approve / Next buttons in the footer enable bulk review without dismissing the dialog between rows. All data sources flow through pure extractors in \code{converter/authority/biodata.py} emitting a common \code{BioData} dataclass, so adding a new source (LC NAF, GND) requires only one extractor function.
...
\noindent The \code{output.ttl} file is the canonical full HMO scholarly graph. Optional publication targets, including an HMO Wikibase deployment and the public Wikidata projection, are generated from this TTL representation and do not replace it as the ontology-native source of record.
...
\hline
\code{hm:external\_identifier\_nli} & P8189 (external-id) & \code{owl:equivalentProperty} \\
\hline
\code{hm:has\_viaf\_id} & P214 (external-id) & \code{owl:equivalentProperty} \\
\hline
\code{hm:has\_wikidata\_id} & --- & \code{owl:sameAs} target \\
\hline
\code{hm:has\_production\_date} & P571 (inception) & Century precision from Hebrew ordinals \\
\hline
\code{hm:has\_genre} & P136 (genre) & 50 genre$\to$QID mappings \\
\hline
\code{hm:has\_subject} & P921 (main subject) & 30 LCSH + 13 Bible + 14 Talmud QIDs \\
\hline
\code{hm:has\_author} & P50 (author) & Via VIAF/Mazal authority resolution \\
\hline
\end{longtable}
...
\subsection{Pipeline Run Artefacts}

Each pipeline run writes a sequence of intermediate JSON artefacts beside the chosen output directory. These are produced per run rather than committed to the repository; the table lists the artefacts referenced elsewhere in this document.
...
\hline
\endfoot
\filepath{marc\_extracted.json} & Stage~1 output: typed \code{ExtractedData} per record. Also read by the eval-agent as the source-of-truth catalog field reference. \\
\hline
\filepath{ner\_results.json} & Stage~2 output: merged NER entities (person / provenance / contents) with \code{source} discriminator. An eval-agent target. \\
\hline
\filepath{authority\_enriched.json} & Stage~3 output: entities annotated with authority matches (Mazal / VIAF / Wikidata / KIMA) and identifiers. Input to Stage~4 and an eval-agent target for the \code{authority} evaluator. \\
\hline
\filepath{authority\_enriched\_reviewed.json} & Curator-reviewed variant of the above; preferred Stage~4 input when present. \\
\hline
\filepath{approvals.json} & Shared approval store: the single source of truth for which extracted entities and authority matches the curator has approved. Keyed canonically by control number, entity group, role/type, and normalised text. Read/written by the NER editor, the authority editor, and the AI-verdict review surface, kept in sync by a live file watcher. The Wikidata-export gate projects only approved entities. \\
\hline
\filepath{output.ttl} & Stage~4/5 output: the canonical full HMO scholarly RDF graph and preferred Stage~6 input. \\
\hline
\filepath{*\_wikidata\_verified.json} & Wikidata Studio review state (round-trippable validation snapshot); never used as manuscript-source input for RDF construction. \\
\hline
\end{longtable}
...
\hline
\endfoot
\filepath{pipeline/models/hebrew-manuscript-joint-ner-v2/model.safetensors} & 701\,MB & Person NER v3 role-aware BIO model (80.4\% span+role F1 on representative benchmark) \\
\hline
...
The final stage of the pipeline projects validated HMO RDF data to Wikidata, making Hebrew manuscript metadata accessible as part of the global LOD ecosystem while preserving the richer ontology-native graph as the scholarly master output. This enables:
...
Stage~6 is therefore a publication/projection stage, not a redefinition of the HMO graph. The Stage~4/5 \code{output.ttl} remains canonical for the complete HMO scholarly model. A project-controlled HMO Wikibase export exposes the same model through Wikibase pages, statements, and edit workflows for curation, but it remains a browsable/editable deployment layer over the ontology graph, not a replacement for \code{hebrew-manuscripts.ttl} or the generated \code{output.ttl}. The current desktop implementation first creates offline Wikibase drafts and a schema bootstrap rather than writing directly to the cloud instance; live upload is deliberately gated behind schema review, connection testing, and later curator approval. The export is full at the entity/statement level, while multi-class RDF typing remains a known schema-fidelity improvement: the JSON draft records a preferred \code{class\_uri} today and should add \code{class\_uris} before treating the draft as a complete type-level mirror of the ontology. Wikidata remains the richest safe public projection because it must conform to Wikidata's community data model, property constraints, notability expectations, and upload safety rules.
...
\hline
\endfoot
F4\_Manifestation\_Singleton & Q87167 (manuscript) & P195 (collection), P17 (country: Israel), P131 (located in: Jerusalem), P921 (main subject), P186 (material), P1104 (pages), P127 (owned by), P6216 (copyright), P8189 (NLI ID, external-id), P571 (inception), P1071 (location), P11603 (transcribed by) \\
\hline
F1\_Work & Q47461344 (written work) & P50 (author), P407 (language of work), P136 (genre, 50 QID mappings) \\
\hline
E21\_Person & Q5 (human) & P569/P570 (birth/death), P106 (occupation), P214 (VIAF, external-id), P8189 (NLI ID, external-id), P21 (gender), P1343 (described by Ktiv), P1412 (Hebrew language), P1559 (native name), P227 (GND), P244 (LCCN), P213 (ISNI), P268 (BnF) \\
\hline
E53\_Place & Q515 (city) etc. & P625 (coordinates), P17 (country) \\
\hline
Production event & qualifier on manuscript & P571 (inception, 96\% coverage with Hebrew century parsing), P1071 (location of creation), P170 (creator) \\
\hline
\end{longtable}

\subsection{Upload Methods}

Two upload methods are implemented:

\subsubsection{Live Upload via WikibaseIntegrator}

For programmatic upload, the pipeline uses the \code{wikibaseintegrator} library, which wraps the Wikidata API with proper session management, retry logic, and rate limiting. The \code{WikidataUploader} class supports four authentication methods, auto-detected from the token format:

\begin{description}[style=nextline, leftmargin=2.5cm]
\item[Bot password] \code{Username@BotName:password} --- uses \code{wbi\_login.Login}.
\item[OAuth 2.0 owner-only] \code{consumer\_key|consumer\_secret} (pipe-separated) --- uses \code{wbi\_login.OAuth2}. Recommended for registered owner-only consumers.
\item[OAuth 1.0a] \code{consumer\_key|consumer\_secret|access\_token|access\_secret} (four pipe-separated parts) --- uses \code{wbi\_login.OAuth1}.
\item[Pre-issued JWT bearer] \code{eyJ0eXAiOiJKV1QiLCJhbGci...} (three dot-separated base64url parts, starting with \code{eyJ}). Issued directly by \url{meta.wikimedia.org} when the OAuth consumer registration is ``owner-only'' + ``Client is confidential.'' The uploader wraps a \code{requests.Session} with the header \code{Authorization: Bearer <jwt>} and hands it to \code{wbi\_login.\_Login}; no consumer-secret exchange is performed.
\end{description}

Recommended scopes for a pipeline consumer are exactly four: \emph{basic}, \emph{high-volume (bot) access}, \emph{edit existing pages}, \emph{create/edit/move pages}. Every additional scope expands the blast radius of a token leak without enabling any pipeline feature.

Rate limiting is enforced at two levels:
\begin{itemize}
\item \textbf{Per-edit delay:} 1.5 seconds between API writes ($\sim$40 edits/minute), safe for OAuth applications with a 5,000 requests/hour quota.
\item \textbf{Batch pause:} When batch mode is enabled (default for live uploads), the uploader pauses 30 seconds every 45 items to avoid Wikidata server throttling. WikibaseIntegrator's internal backoff is capped at 30 seconds (overriding the default 3,600-second maximum).
\end{itemize}

\subsubsection{QuickStatements (Dry Run)}

For batch review before upload, the dry-run mode exports a QuickStatements-format text file. The exporter emits three section banners in strict dependency order---\textbf{persons first} (because work items reference them via P50), \textbf{works second} (because manuscripts reference them via P1574 \textit{exemplar of}), \textbf{manuscripts last}---so QuickStatements can resolve every \code{LAST} back-reference within the same batch:

\begin{lstlisting}[basicstyle=\ttfamily\small, frame=single]
/* Persons (create first; works and manuscripts reference them) */
CREATE
LAST  Lhe   "משה בן מימון"
LAST  Len   "Maimonides"
LAST  P31   Q5

/* Works (create second; manuscripts P1574 them) */
CREATE
LAST  Lhe   "תקנות רבנו גרשם מאור הגולה"
LAST  Len   "Takanut rivno gereshem meor hagola (NLI 990001801390205171)"
LAST  P31   Q47461344
...
\begin{enumerate}
\item \textbf{Authority-based reconciliation:} Instead of SPARQL reconciliation (which proved too slow and unreliable at scale), items with existing QIDs from authority matching (VIAF P214, NLI P8189) are updated; items without authority-linked QIDs are created as new entities.
\item \textbf{Two-phase pipeline:} Phase~1 preferably builds \code{WikidataItem} objects from the validated HMO \code{output.ttl} via the HMO-to-Wikidata crosswalk. When \code{authority\_enriched\_reviewed.json} exists beside the RDF file, it is used as a reviewed provenance sidecar so the projection preserves curator-approved authority matches and the current rich Wikidata mappings. The older JSON-only \code{WikidataItemBuilder} path remains as a compatibility fallback. Phase~2 uploads to Wikidata or exports QuickStatements.
\item \textbf{Projection coverage reporting:} Stage~6 emits a coverage report that distinguishes HMO nodes projected directly to Wikidata items/statements, nodes represented indirectly through qualifiers or summaries, and ontology-native structures intentionally omitted from Wikidata.
\item \textbf{Interactive validation workbench:} Between Phase~1 (build) and Phase~2 (upload) the \emph{Wikidata Studio} exposes a curator-facing review step: every item is run through the \code{\_ValidationWorker} (reconciler + per-PID collision + SPARQL existence + three-channel creator check + in-process validator re-run), and the curator approves each row individually. A ``💾 Save validation'' action exports \code{<input\_stem>\_wikidata\_verified.json}, which round-trips through the Load field so multi-session review is supported on large corpora. Pagination (25/50/100 per page) keeps the table responsive at 5\,000+ items.
...
\item \textbf{Incremental upload:} Start with pilot manuscripts, then expand to the full catalog.
...
\begin{itemize}
\item \textbf{Duplicate detection:} Some Hebrew manuscripts may already have Wikidata items; authority-based reconciliation via VIAF and NLI identifiers is the primary deduplication method.
...
\item \textbf{Full-model deployment:} Project-controlled HMO Wikibase can represent the full scholarly model more completely than Wikidata. The pipeline therefore exports full-HMO Wikibase drafts from \code{output.ttl} and keeps a separate Wikidata projection for the public global graph.
\item \textbf{Community standards:} Upload must conform to Wikidata's notability guidelines and data quality expectations.
\item \textbf{Rate limiting:} The Wikidata API enforces strict rate limits; the pipeline uses per-edit delays and batch pauses to stay within limits.
\end{itemize}
...
\item Role-aware person BIO token classification replacing the legacy keyword fallback for bundled models
...
\begin{itemize}
\item Wikidata upload module implemented: \code{WikidataItemBuilder}, \code{QuickStatementsExporter}, \code{WikidataUploader} classes
\item OAuth~2.0 and bot password authentication via \code{wikibaseintegrator}
\item Rate limiting: 1.5s per-edit delay, 30s batch pause every 45 items (batch mode ON by default)
\item Authority-based reconciliation replaces SPARQL reconciliation (uses VIAF/NLI IDs from authority matching)
\item Null-safe signal handling and QThread lifecycle management (prevents SIGABRT crashes)
\item \textbf{Property coverage improvements (v1.8):}
  \begin{itemize}
  \item Hebrew century date parsing for P571 (inception): 22\%$\to$96\% of MSS
  \item 50 genre$\to$QID mappings for P136: 14\%$\to$100\% of MSS with genre data
  \item 30 LCSH subject$\to$QID mappings for P921: 56$\to$91 claims (46\% of MSS)
  \item External-id type fix for P8189 (NLI) and P214 (VIAF)
  \item Type guards for P5816/P527/P195 (skip invalid value types)
  \item Average 20.9 statements per manuscript on 100-record pilot
  \end{itemize}
...
\hline
\code{wikibaseintegrator} & Wikidata & Wikidata API client with OAuth~2.0/1.0a login, retry, and rate limiting \\
\hline
...
\hline
Wikidata API & \code{www.wikidata.org/w/api.php} & OAuth~2.0 (\code{consumer\_key|consumer\_secret}) or bot password \\
\hline
