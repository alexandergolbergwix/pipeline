...
\item[Stage 6: Wikidata Upload]
  The validated HMO RDF graph is projected into Wikidata statements and uploaded via the Wikidata API and/or QuickStatements. The RDF graph remains the full scholarly representation, while Wikidata is the richest safe public projection. For backward compatibility the legacy JSON-based builder remains available, but the preferred Stage~6 input is the Stage~4/5 \code{output.ttl} generated from reviewed authority data.
\end{description}
...
For batch review before upload, the dry-run mode exports a QuickStatements-format text file:
