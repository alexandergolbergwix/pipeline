...
\item \textbf{Named Entity Recognition (NER) System} --- a two-stage deep-learning pipeline (HalleluBERT multi-entity NER at 88.71\% F1 + NeoDictaBERT role classifier at 90.64\% accuracy) that extracts person names and their roles (author, scribe, owner, censor, translator, commentator) from unstructured Hebrew note fields (MARC 500, 520, 545, 546, 561, 957).
\item \textbf{Authority File Integration} --- matching extracted entities against the NLI Authority File (Mazal), VIAF, Library of Congress Name Authority File (LCNAF), and GeoNames/Wikidata for geographic enrichment.
...
\textbf{The NER System (Paper 2).} The Named Entity Recognition system uses distant supervision on MARC structured fields to train models that extract person names from unstructured note fields. The two-stage architecture---multi-entity NER followed by role classification---identifies persons and classifies their roles (AUTHOR, TRANSCRIBER, OWNER, CENSOR, TRANSLATOR, COMMENTATOR) with a combined end-to-end accuracy of approximately 80\%.
...
\node[note, below=0.15cm of ner] {HalleluBERT\\+ NeoDictaBERT};
\node[note, below=0.15cm of auth] {Mazal, VIAF,\\LCNAF, GeoNames};
...
\item[Stage 2: NER Extraction]
  Unstructured text from note fields (500, 520, 545, 546, 561, 957) is passed to the NER inference pipeline. The HalleluBERT multi-entity model identifies person name spans; the NeoDictaBERT role classifier assigns a role category to each extracted person. Post-processing rules clean up edge cases.

\item[Stage 3: Authority Resolution]
  Extracted entity names (persons, places, works) are matched against multiple authority sources: the NLI Mazal authority file (local SQLite database with fuzzy matching), VIAF (API-based), Library of Congress Name Authority File (API-based), and GeoNames/Wikidata (for geographic entities). Matched entities receive stable global identifiers.
...
\node[proc] (ner) at (4,-5.4) {NER Pipeline};
\node[data] (persons) at (4,-7.2) {Persons + Roles};
\node[ext] (mazal) at (8,-3.6) {Mazal DB};
\node[ext] (viaf) at (8,-4.6) {VIAF API};
\node[ext] (geonames) at (8,-5.6) {GeoNames};
...
\item \code{ner.inference\_pipeline.ProductionNERPipeline} --- NER + role classification
...
The NER extraction system addresses a fundamental limitation of MARC records: critical information about persons associated with manuscripts---scribes, owners, censors, translators, commentators---is often mentioned only in free-text note fields rather than in structured person fields. The system extracts person names from these unstructured Hebrew texts and classifies their roles.

\textbf{Input:} Hebrew text from MARC note fields (500, 520, 545, 546, 561, 957).\\
\textbf{Output:} Extracted person entities with role classifications and confidence scores.

Example:
\begin{lstlisting}[basicstyle=\ttfamily\small, frame=none, backgroundcolor=\color{white}]
Input:  "Written by Moses ben Jacob and copied by David ben Samuel"
Output: Person 1: Moses ben Jacob, Role: TRANSCRIBER, Confidence: 100%
        Person 2: David ben Samuel, Role: TRANSCRIBER, Confidence: 100%
\end{lstlisting}
...
\node[box, fill=yellow!15] (ner) at (5,0) {\textbf{Stage 1: NER}\\HalleluBERT\\Multi-Entity};
\node[box, fill=green!12] (sent) at (10,0) {\textbf{Stage 1.5}\\Sentence\\Extraction};
\node[box, fill=blue!12] (role) at (5,-3) {\textbf{Stage 2: Role}\\NeoDictaBERT\\Classifier};
\node[box, fill=purple!10] (post) at (10,-3) {\textbf{Stage 3}\\Post-Processing\\Rules};
\node[box, fill=red!8] (out) at (10,-6) {Persons\\+ Roles\\+ Confidence};

\draw[arrow] (text) -- (ner);
\draw[arrow] (ner) -- (sent);
\draw[arrow] (sent) -- ++(0,-1.2) -| (role);
\draw[arrow] (role) -- (post);
\draw[arrow] (post) -- (out);

\node[note, below=0.1cm of ner] {88.71\% F1};
\node[note, below=0.1cm of role] {90.64\% accuracy};

\end{tikzpicture}
\caption{Two-stage NER architecture with sentence extraction and post-processing.}
\label{fig:ner-architecture}
\end{figure}

\subsubsection{Stage 1: Multi-Entity NER}

The NER model is based on HalleluBERT, a BERT model pre-trained on historical Hebrew texts. It has been fine-tuned for token classification with BIO labelling (B-PERSON, I-PERSON, O) on a dataset of 7,580 samples derived through distant supervision from 123,621 MARC records.
...
\item F1 score: 88.71\% (best among tested models)
\end{itemize}

\subsubsection{Stage 1.5: Sentence Extraction}

After NER identifies person spans, the system extracts the sentence context surrounding each person. This focused context is passed to the role classifier, improving classification accuracy by reducing noise from irrelevant text.

\subsubsection{Stage 2: Role Classification}

The role classifier is a NeoDictaBERT model fine-tuned for sequence classification with a two-input format: \code{[PERSON: name] + sentence context}. It classifies each person into one of six roles:
...
\subsubsection{Stage 3: Post-Processing Rules}

The \code{PostProcessingRules} module applies rule-based corrections to NER output, handling edge cases such as:
\begin{itemize}
...
\midrule
NER & DictaBERT (baseline) & 80.42\% F1 & General Hebrew \\
NER & HalleluBERT (single) & 86.56\% F1 & 8,186 single-person \\
NER & NeoDictaBERT (single) & 86.09\% F1 & 8,186 single-person \\
\textbf{NER} & \textbf{HalleluBERT (multi)} & \textbf{88.71\% F1} & \textbf{7,580 mixed} \\
\midrule
Role & HalleluBERT (two-input) & 89.36\% acc & --- \\
\textbf{Role} & \textbf{NeoDictaBERT (two-input)} & \textbf{90.64\% acc} & --- \\
\midrule
\multicolumn{2}{l}{\textbf{Combined end-to-end}} & \textbf{$\sim$80.43\%} & --- \\
\bottomrule
\end{tabular}
\caption{NER and role classification model comparison.}
\label{tab:ner-models}
\end{table}

\subsection{Training Data: Distant Supervision}

The NER training dataset was created using distant supervision: structured person fields from MARC (100\$a, 600\$a, 700\$a, 800\$a) were matched against concatenated note fields (500\$a, 520\$a, 545\$a, 546\$a, 561\$a, 957\$a) using fuzzy string matching. This produced labelled training data without manual annotation.
...
\midrule
Multi-entity NER (filtered) & 6,058 & 757 & 765 \\
Role classification (6-class) & varies & varies & varies \\
K-fold (5 folds) & \multicolumn{3}{c}{5 $\times$ train/val splits} \\
\bottomrule
\end{tabular}
\caption{NER training dataset splits.}
\label{tab:ner-data}
\end{table}

Dataset composition: 85.4\% single-person samples, 10.1\% multi-person (639 with 2 persons, 152 with 3, 32 with 4+).

\subsection{MARC Fields Targeted by NER}

\begin{longtable}{|l|l|p{7cm}|}
\hline
\textbf{MARC Field} & \textbf{Subfield} & \textbf{Content Type} \\
\hline
\endfirsthead
\hline
\textbf{MARC Field} & \textbf{Subfield} & \textbf{Content Type} \\
...
\hline
\endfoot
500 & \$a & General notes (most common NER target) \\
\hline
520 & \$a & Summary/abstract notes \\
\hline
545 & \$a & Biographical or historical notes \\
\hline
546 & \$a & Language notes \\
\hline
561 & \$a & Ownership and custodial history \\
\hline
957 & \$a & Local NLI colophon transcription \\
\hline
\end{longtable}

\subsection{Integration Point}

The NER system is invoked through the \code{ProductionNERPipeline} class:

\begin{lstlisting}[language=Python, caption={NER pipeline invocation}]
from ner.inference_pipeline import ProductionNERPipeline

pipeline = ProductionNERPipeline(
    ner_model_path="ner/hallelubert_multi_entity/checkpoint-6822",
    classifier_model_path="ner/neodictabert_role_classifier",
    use_postprocessing=True
)

results = pipeline.process(text)
# Returns: [{"person": "name", "role": "TRANSCRIBER", 
#            "confidence": 0.97, "spans": [...]}]
\end{lstlisting}
...
\subsection{GeoNames / Wikidata Geographic Enrichment (Planned)}

Geographic names---production places, ownership locations, current repositories---will be enriched through:

\begin{itemize}
\item \textbf{GeoNames API} (\code{https://api.geonames.org/}): Resolving place names to geographic coordinates, administrative hierarchies, and GeoNames IDs.
\item \textbf{Wikidata API} (\code{https://www.wikidata.org/w/api.php}): Resolving places, persons, and works to Wikidata QIDs, enabling integration with the global knowledge graph.
\end{itemize}

\subsection{Resolution Strategy}

The planned authority resolution strategy follows a cascading priority:

\begin{enumerate}
\item \textbf{Local (Mazal):} Query the NLI authority file first (fastest, domain-specific).
\item \textbf{VIAF:} If no Mazal match, query VIAF for international coverage.
\item \textbf{Wikidata:} If no VIAF match, attempt Wikidata reconciliation.
\item \textbf{Unresolved:} If no match is found, the entity is flagged for manual review.
\end{enumerate}
...
\hline
\filepath{pipeline/scripts/build\_mazal\_index.py} & --- \\
\hline
\end{longtable}
...
\hline
\endfoot
\filepath{pipeline/ner/joint\_entity\_role\_model/best\_model.pt} & 2.1\,GB & Joint NER+role model \\
\hline
...
Currently, the pipeline components exist as separate modules with partial integration:
...
\item The NER system operates as a standalone pipeline with its own input/output format.
\item VIAF, LCNAF, and GeoNames integration exists only at schema level (properties defined, not populated).
\item Wikidata upload is not yet implemented.
\end{itemize}

The target state is a unified pipeline where all components are orchestrated by a single entry point.

\subsection{NER Integration Architecture}

The NER system must be integrated at two points:

\subsubsection{Point 1: Note Field Extraction}

After \code{extract\_all\_data()} produces an \code{ExtractedData} instance, the \code{notes} list and \code{colophon\_text} are concatenated and passed to the NER pipeline:

\begin{lstlisting}[language=Python, caption={Planned NER integration point}]
extracted = extract_all_data(record)

# Collect all note text for NER
note_texts = []
if extracted.notes:
    note_texts.extend(extracted.notes)
if extracted.colophon_text:
    note_texts.append(extracted.colophon_text)
if extracted.provenance:
    note_texts.append(extracted.provenance)

# Run NER on concatenated notes
for text in note_texts:
    ner_results = ner_pipeline.process(text)
    for result in ner_results:
        extracted.contributors.append({
            'name': result['person'],
...
NER-extracted persons are then passed through the authority resolution cascade:

\begin{lstlisting}[language=Python, caption={Authority resolution for NER-extracted persons}]
for contributor in extracted.contributors:
    if contributor.get('source') == 'ner_extraction':
        nli_id = mazal_matcher.match_person(contributor['name'])
        if nli_id:
            contributor['nli_id'] = nli_id
        else:
            viaf_id = viaf_client.search_person(contributor['name'])
            if viaf_id:
                contributor['viaf_id'] = viaf_id
\end{lstlisting}
...
\hline
GeoNames API & Rate limit / error & Queue for retry \\
\hline
...
\subsection{Phase 1: Core Pipeline (Current)}
...
\item NER inference pipeline (implemented, standalone)
\end{itemize}

\subsection{Phase 2: NER Integration and Authority Enrichment (Next)}

\begin{itemize}
\item Integrate NER pipeline into the MARC-to-RDF flow
\item Implement VIAF API client with caching
\item Implement GeoNames API client for place resolution
\item Build OpenRefine reconciliation workflow for batch entity matching
\item Implement Wikidata reconciliation API client
\end{itemize}
...
\subsection{Phase 4: Wikidata Upload}

\begin{itemize}
\item Develop Wikidata upload module (API + QuickStatements)
\item Reconcile against existing Wikidata items
...
\hline
GeoNames & \code{api.geonames.org/} & API key (free tier) \\
\hline
