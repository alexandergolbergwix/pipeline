...
For programmatic upload, the pipeline uses the \code{wikibaseintegrator} library, which wraps the Wikidata API with proper session management, retry logic, and rate limiting. The \code{WikidataUploader} class supports three authentication methods:
...
\item[OAuth 2.0] \code{consumer\_key|consumer\_secret} (pipe-separated) --- uses \code{wbi\_login.OAuth2}. This is the recommended method for registered Wikidata applications.
