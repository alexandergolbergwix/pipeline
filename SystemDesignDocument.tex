...
\item[Distribution] \textbf{uv bootstrap + native OS installers} --- the installer ships a small launcher ($<$20\,MB) that invokes a \code{uv}-based bootstrap script to install Python~3.12 and all dependencies into an isolated virtual environment. Five ML models are bundled in the \code{.app}/\code{.dmg}: Person NER ($\approx$2.2\,GB, HuggingFace snapshot), Provenance NER ($\approx$704\,MB), Contents NER ($\approx$704\,MB), MARC~500 Colophon~ML sentence classifier ($\approx$703\,MB), and Genre~ML classifier ($\approx$703\,MB, used by Stage~3 for P136 fallback). On first launch any missing weights fall through to a PyQt6 wizard using \code{huggingface\_hub}. The macOS \code{.app} bundle is $\approx$16\,GB uncompressed; the compressed DMG is $\approx$11\,GB.
