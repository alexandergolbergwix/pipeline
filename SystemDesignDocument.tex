...
\item[Distribution] \textbf{Hybrid packaging by platform} --- macOS uses a native app bundle / DMG workflow, while Windows uses a fully offline PyInstaller one-folder build wrapped by Inno Setup into a single \code{.exe}. Five ML models are bundled with the installed application on both platforms: Person NER ($\approx$2.2\,GB, HuggingFace snapshot), Provenance NER ($\approx$704\,MB), Contents NER ($\approx$704\,MB), MARC~500 Colophon~ML sentence classifier ($\approx$703\,MB), and Genre~ML classifier ($\approx$703\,MB, used by Stage~3 for P136 fallback). The Windows source bundle used to prepare the offline installer is $\approx$6\,GB compressed and expands to $\approx$9.5\,GB before Inno Setup compression; the resulting \code{MHMPipeline-Setup-0.1.0.exe} is $\approx$4--5\,GB.
