...
\item[Page 2 --- Model Profile] Displays the three NER models that will be downloaded:
  \begin{itemize}
    ...
    \item \textbf{Provenance NER model} (\filepath{ner/provenance\_ner\_model.pt}, $\approx$704\,MB): DictaBERT + token-classification head trained on MARC 561 provenance fields; extracts OWNER, DATE, and COLLECTION entities (93.96\% F1).
...
\hline
\code{WikidataPanel} & OAuth token field (masked), dry-run checkbox, entity reconciliation options, ``Upload to Wikidata'' button & \code{log\_line} $\to$ log; \code{progress} $\to$ upload bar \\
\hline
\end{longtable}
