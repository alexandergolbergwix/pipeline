...
\hline
\code{WikidataStudioPanel} (supersedes the separate \code{WikidataPreviewPanel} and \code{WikidataUploadPanel} from v1.x) & Six-step stepper (Load $\to$ Build $\to$ Browse $\to$ Check $\to$ Approve $\to$ Upload); liquid-glass ``\emph{Credentials}'' card (token field masked, show/hide toggle, moratorium-status banner) accepting four formats --- bot password \code{User@Bot:pass}, OAuth~2.0 owner-only consumer \code{key|secret}, OAuth~1.0a \code{key|secret|token|secret}, and a pre-issued JWT bearer token \code{eyJ...} (auto-detected by format and routed through a \code{requests.Session} with \code{Authorization: Bearer}); dry-run checkbox; batch mode checkbox (ON by default, 45~items + 30s pause); \code{QTabWidget} with \code{QPEntityBrowser} (editable Q/P entities with validator Issues column, auto-approve, per-row claim-edit dialog) and an RDF-Triples preview; full-screen popup; ``Check Wikidata for duplicates'' and ``Export / Upload approved'' buttons & \code{log\_line} $\to$ log; \code{progress} $\to$ upload bar; \code{entity\_status} $\to$ per-entity progress view; \code{upload\_requested(Path,Path,str,bool,bool,list)} carries only the approved-item subset \\
\hline
\end{longtable}
...
Dialogs that currently use the backdrop: \code{ClaimsEditDialog}, \code{ItemDetailDialog}, \code{ItemEditDialog}, \code{AutoApproveDialog}, plus the full-screen popup of \code{WikidataStudioPanel} and the credentials card on that panel. New dialogs must either inherit \code{GlassDialog} or call \code{install\_glass\_backdrop(self)}; the \code{reinstall-app} skill contains a grep-audit that fails if a \code{QDialog} subclass is added without the backdrop.
