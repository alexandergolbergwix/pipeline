...
\item[Page 2 --- Model Profile] Displays the single joint NER model that will be downloaded:
  \begin{itemize}
    \item \textbf{Joint model} (\code{alexgoldberg/hebrew-manuscript-joint-ner-v2}, $\approx$2.2\,GB): DictaBERT-based multi-task model that performs PERSON entity detection (NER F1 0.9080) and role classification (AUTHOR / COPYIST / OTHER) in a single forward pass; runs on MPS, CUDA, or CPU.
  \end{itemize}
...
\node[layer, fill=green!10] (core) at (0,-4.0)
  {\textbf{Core Pipeline Layer (existing, unchanged)}\\
   ...
   \code{GraphBuilder} \quad \code{ShaclValidator}};
...
\hline
\code{AuthorityWorker} & Matches NER entities against Mazal (SQLite) + VIAF (REST); matches MARC places against KIMA (SQLite) & Worker & Core authority module \\
\hline
...
\foreach \y/\name/\col in {
  7.3/1. Parse MARC/gray!30,
  6.5/2. NER/gray!30,
  5.7/3. Authority/gray!30,
  4.9/4. Build RDF/gray!30,
  4.1/5. Validate/gray!30,
  3.3/6. Wikidata/gray!30} {
...
\hline
\endfoot
\code{ConvertPanel} & MARC file picker, output dir picker, record range spin boxes, Run button & \code{progress} $\to$ progress bar; \code{log\_line} $\to$ log viewer \\
\hline
\code{NerPanel} & Input JSON selector, output dir selector, model ID text field (\code{alexgoldberg/hebrew-manuscript-joint-ner-v2}), batch size spin box, ``Load Results'' button for viewing previously generated NER JSON & \code{progress} $\to$ per-record bar; \code{log\_line} $\to$ log; \code{finished} $\to$ \code{display\_entities()} \\
\hline
\code{AuthorityPanel} & NER results selector, output dir, MARC extract selector, VIAF/KIMA checkboxes, Mazal DB + XML dir selectors, KIMA DB + TSV dir selectors, ``Rebuild Mazal Index\ldots'' and ``Rebuild KIMA Index\ldots'' buttons & \code{run\_requested(Path,Path,Path,bool,bool,str,str)} $\to$ controller; rebuild buttons launch \code{MazalIndexWorker}/\code{KimaIndexWorker} in-panel \\
\hline
\code{RdfPanel} & Namespace prefix editor, URI template fields, serialisation format combo & \code{progress} $\to$ bar; \code{finished} $\to$ open TTL in preview widget \\
\hline
\code{ValidatePanel} & SHACL shapes file selector, severity filter combo, violations table view & \code{finished} $\to$ populate \code{QTableWidget} with SHACL report \\
\hline
\code{WikidataPanel} & OAuth token field (masked), dry-run checkbox, entity reconciliation options & \code{log\_line} $\to$ log; \code{progress} $\to$ upload bar \\
\hline
\end{longtable}
...
\item \textbf{TripleGraphView} --- transparent background inherits palette; edges use \code{palette(mid)}
...
\hline
\code{log\_level} & \code{str} & \code{INFO} & Python logging level \\
\hline
...
\hline
Launcher & \code{.app} bundle + shell script & \code{.exe} + \code{.bat} & Installer-generated \\
\hline
...
\hline
Disk (models) & 2.2\,GB & 2.2\,GB & Single joint NER model downloaded at first run \\
\hline
