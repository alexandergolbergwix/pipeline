...
class ExtractedData:
    ...
    sfardata_id: str | None = None  # From 024
    shelfmark: str | None = None  # From 942
    institution: str | None = None  # From 942
    host_record_id: str | None = None  # From 773
    ...
    acquisition_source: str | None = None  # From 541
...
def parse_date_string(date_str: str) -> dict[str, Any]:
    ...
    century_patterns = [
        (r'י"ח', 18),
        (r"י״ח", 18),  # 18th century
        (r'י"ז', 17),
        (r"י״ז", 17),  # 17th century
        (r'ט"ז', 16),
        (r"ט״ז", 16),  # 16th century
        (r'ט"ו', 15),
        (r"ט״ו", 15),  # 15th century
        (r'י"ד', 14),
        (r"י״ד", 14),  # 14th century
        (r'י"ג', 13),
        (r"י״ג", 13),  # 13th century
        (r'י"ב', 12),
        (r"י״ב", 12),  # 12th century
        (r'י"א', 11),
        (r"י״א", 11),  # 11th century
        (r"י'", 10),
        (r"י׳", 10),  # 10th century
    ]
...
def parse_extent(extent_str: str) -> int | None:
...
def build_rdf_graph(data: ExtractedData) -> Graph:
    ...
    # Identifiers
    graph.add(
        (ms_uri, HM.external_identifier_nli, Literal(data.control_number, datatype=XSD.string))
    )
    if data.sfardata_id:
    ...
    if data.date_start or data.date_end:
        time_label = (
            ...
            else f"{data.date_start}-{data.date_end}"
        )
        ...
        if data.date_start:
            graph.add(
                (
                    ...
                    Literal(data.date_start, datatype=XSD.integer),
                )
            )
        if data.date_end:
            graph.add(
                (time_uri, HM.latest_possible_date, Literal(data.date_end, datatype=XSD.integer))
            )
        ...
        graph.add(
            (
                ...
                Literal(
                    bool(data.date_start and data.date_end and data.date_start == data.date_end),
    ...
    if data.title:
        ...
        if data.subtitle:
            graph.add(
                (work_uri, HM.has_title, Literal(f"{data.title} : {data.subtitle}", lang="he"))
            )
        ...
        # Expression
        expression_uri = HM[
            f"Expression_{normalize_string(data.title)}_in_{normalize_string(data.control_number)}"
        ]
        ...
        graph.add(
            (
                expression_uri,
                RDFS.label,
                Literal(f"{data.title} (in MS {data.control_number})", lang="he"),
            )
        )
        ...
        graph.add(
            (
                main_cu_uri,
                RDFS.label,
                Literal(f"Main codicological unit of MS {data.control_number}", lang="en"),
            )
        )
    ...
    for author in data.authors:
        ...
        if "birth_year" in author:
            graph.add(
                (
                    ...
                    Literal(author["birth_year"], datatype=XSD.integer),
                )
            )
        if "death_year" in author:
            graph.add(
                (
                    ...
                    Literal(author["death_year"], datatype=XSD.integer),
                )
            )
        if author.get("external_uri_nli"):
            graph.add(
                (
                    person_uri,
                    HM.external_uri_nli,
                    Literal(author["external_uri_nli"], datatype=XSD.anyURI),
                )
            )
        ...
        if author.get("wikidata_id"):
            graph.add(
                (person_uri, HM.wikidata_id, Literal(author["wikidata_id"], datatype=XSD.string))
            )
        ...
        if work_uri:
            creation_uri = HM[
                f"WorkCreation_{normalize_string(data.title)}_by_{normalize_string(author['name'])}"
            ]
    ...
    for contrib in data.contributors:
        ...
        if "birth_year" in contrib:
            graph.add(
                (
                    entity_uri,
                    CIDOC.P82a_begin_of_the_begin,
                    Literal(contrib["birth_year"], datatype=XSD.integer),
                )
            )
        if "death_year" in contrib:
            graph.add(
                (
                    entity_uri,
                    CIDOC.P82b_end_of_the_end,
                    Literal(contrib["death_year"], datatype=XSD.integer),
                )
            )
        if contrib.get("external_uri_nli"):
            graph.add(
                (
                    entity_uri,
                    HM.external_uri_nli,
                    Literal(contrib["external_uri_nli"], datatype=XSD.anyURI),
                )
            )
        ...
        if contrib.get("wikidata_id"):
            graph.add(
                (entity_uri, HM.wikidata_id, Literal(contrib["wikidata_id"], datatype=XSD.string))
            )
    ...
    for i, content in enumerate(data.contents, 1):
        ...
        content_expr_uri = HM[
            f"Expression_{normalize_string(content['title'])}_in_{normalize_string(data.control_number)}"
        ]
        ...
        graph.add(
            (
                content_expr_uri,
                RDFS.label,
                Literal(f"{content['title']} (in MS {data.control_number})", lang="he"),
            )
        )
        ...
        if content.get("sequence"):
            ...
            graph.add(
                (pos_bnode, HM.anthology_order, Literal(content["sequence"], datatype=XSD.integer))
            )
        ...
        graph.add(
            (
                cu_uri,
                RDFS.label,
                Literal(f"Codicological unit {seq_value} of MS {data.control_number}", lang="en"),
            )
        )
        ...
        if content.get("folio_range"):
            graph.add(
                (cu_uri, HM.has_folio_range, Literal(content["folio_range"], datatype=XSD.string))
            )
    ...
    for add_title in data.additional_titles:
        ...
        add_expr_uri = HM[
            f"Expression_{normalize_string(add_title)}_in_{normalize_string(data.control_number)}"
        ]
    ...
    if not structural_cu_uris:
        ...
        graph.add(
            (
                default_cu_uri,
                RDFS.label,
                Literal(f"Codicological unit 1 of MS {data.control_number}", lang="en"),
            )
        )
    ...
    for idx in range(pu_count):
        ...
        graph.add(
            (
                pu_uri,
                RDFS.label,
                Literal(f"Paleographical unit {idx + 1} of MS {data.control_number}", lang="en"),
            )
        )
        ...
        if idx < len(scribe_entity_uris):
            ...
            pu_scribe_uri = HM[
                f"Unknown_Scribe_{normalize_string(data.control_number)}_{idx + 1:02d}"
            ]
            ...
            graph.add(
                (
                    pu_scribe_uri,
                    RDFS.label,
                    Literal(f"Unknown scribe {idx + 1} (MS {data.control_number})", lang="en"),
                )
            )
    ...
    if data.is_multi_volume:
        ...
        graph.add(
            (
                set_uri,
                RDFS.label,
                Literal(f"Multi-volume set: {data.title or data.control_number}", lang="en"),
            )
        )
    ...
    if data.is_anthology and data.num_works > 1:
        ...
        graph.add(
            (
                anthology_uri,
                RDFS.label,
                Literal(f"Anthology structure of MS {data.control_number}", lang="en"),
            )
        )
        graph.add((ms_uri, HM.has_anthology_structure, anthology_uri))
        graph.add(
            (anthology_uri, HM.number_of_works, Literal(data.num_works, datatype=XSD.integer))
        )
    ...
    if data.has_multiple_hands:
        ...
        graph.add(
            (
                ms_uri,
                RDFS.comment,
                Literal("This manuscript was written by multiple scribes/hands", lang="en"),
            )
        )
    ...
    graph.add(
        (
            cat_view_uri,
            RDFS.label,
            Literal(f"Cataloging view for MS {data.control_number}", lang="en"),
        )
    )
    ...
    graph.add(
        (
            phil_view_uri,
            RDFS.label,
            Literal(f"Philological view for MS {data.control_number}", lang="en"),
        )
    )
    ...
    for pair_index, pair in enumerate(work_expression_pairs, 1):
        ...
        if work_local not in tradition_by_work:
            ...
            graph.add(
                (tradition_uri, RDFS.label, Literal(f"Text tradition for {work_local}", lang="en"))
            )
        else:
        ...
        witness_uri = HM[
            f"TransmissionWitness_{normalize_string(data.control_number)}_{pair_index:02d}"
        ]
        ...
        graph.add(
            (
                ...
                RDFS.label,
                Literal(
                    f"Transmission witness {pair_index} for MS {data.control_number}", lang="en"
                ),
            )
        )
        ...
        graph.add(
            (
                ...
                Literal(
                    "Cataloging and philological alignment for generated pilot data",
                    datatype=XSD.string,
                ),
            )
        )
    ...
    if data.acquisition_source:
        graph.add(
            (ms_uri, HM.acquisition_source, Literal(data.acquisition_source, datatype=XSD.string))
        )
    ...
    for i, restriction in enumerate(data.usage_restrictions, 1):
        ...
        if restriction.get("description"):
            graph.add(
                (
                    restr_uri,
                    HM.restriction_type,
                    Literal(restriction["description"], datatype=XSD.string),
                )
            )
            graph.add(
                (
                    restr_uri,
                    HM.usage_restriction_note,
                    Literal(restriction["description"], datatype=XSD.string),
                )
            )
        ...
        if restriction.get("url"):
            graph.add(
                (restr_uri, HM.restriction_url, Literal(restriction["url"], datatype=XSD.anyURI))
            )
    ...
    for i, rights in enumerate(data.rights_determinations, 1):
        ...
        if rights.get("status"):
            graph.add(
                (rights_uri, HM.rights_status, Literal(rights["status"], datatype=XSD.string))
            )
    ...
    for i, holding in enumerate(data.physical_holdings, 1):
        ...
        if holding.get("institution"):
            graph.add(
                (
                    holding_uri,
                    HM.holding_institution,
                    Literal(holding["institution"], datatype=XSD.string),
                )
            )
            ...
            graph.add(
                (
                    custody_uri,
                    RDFS.label,
                    Literal(f"Transfer of custody {i} for MS {data.control_number}", lang="en"),
                )
            )
        ...
        if holding.get("collection"):
            graph.add(
                (
                    holding_uri,
                    HM.holding_collection,
                    Literal(holding["collection"], datatype=XSD.string),
                )
            )
        ...
        if holding.get("call_number"):
            graph.add(
                (
                    holding_uri,
                    HM.holding_call_number,
                    Literal(holding["call_number"], datatype=XSD.string),
                )
            )
        ...
        if holding.get("availability"):
            graph.add(
                (
                    holding_uri,
                    HM.holding_availability,
                    Literal(holding["availability"], datatype=XSD.string),
                )
            )
        if holding.get("library_name"):
            graph.add(
                (
                    holding_uri,
                    HM.holding_sublibrary,
                    Literal(holding["library_name"], datatype=XSD.string),
                )
            )
    ...
    for i, access in enumerate(data.digital_access_points, 1):
        ...
        if access.get("note"):
            graph.add(
                (access_uri, HM.digital_access_note, Literal(access["note"], datatype=XSD.string))
            )
            graph.add((access_uri, RDFS.label, Literal(access["note"])))
        if access.get("type"):
            graph.add(
                (access_uri, HM.digital_access_type, Literal(access["type"], datatype=XSD.string))
            )
...
def _extract_ontology_iri(ontology_graph: Graph) -> URIRef | None:
    ...
    for line in lines:
        if "owl:AnnotationProperty" in line and line.strip().endswith("."):
            ...
            if prop_uri.endswith(":"):
                prop_uri = prop_uri[:-1]
            expanded = (
                str(graph.namespace_manager.expand_curie(prop_uri))
                if ":" in prop_uri and not prop_uri.startswith("<")
                else prop_uri.strip("<>")
            )
...
def convert_mrc_to_ttl(
    ...
    if ontology_path and ontology_path.exists():
        ...
        _extract_ontology_iri(ontology_graph)
    ...
    if merged_output_path:
        ...
        if merged_removed:
            print(
                f"  Cleaned {merged_removed} conflicting AnnotationProperty declaration(s) in merged output"
            )
...
def main() -> None:
    ...
    convert_mrc_to_ttl(
        input_path, output_file, ontology_path=ontology_file, merged_output_path=merged_output_file
    )
