% Hebrew Manuscripts Ontology Documentation - Hebrew RTL LaTeX
% Based on Prof. Gila Prebor's original design with expert-driven modifications
% Modified/new elements are colored in green
% Compile with XeLaTeX

\documentclass[12pt,a4paper]{article}

% IMPORTANT: Load these packages BEFORE polyglossia (which loads bidi)
\usepackage{xcolor}
\definecolor{newaddition}{RGB}{0,128,0} % Green for new/modified elements

\usepackage{geometry}
\geometry{margin=2.5cm}

% Removed fancyhdr, tocloft, booktabs, enumitem - not in BasicTeX
...
% Hebrew RTL support - load AFTER the above packages
\usepackage{polyglossia}
\setmainlanguage{hebrew}
\setotherlanguage{english}

% Fonts - use fonts available in Overleaf/TeX Live
\usepackage{fontspec}
\defaultfontfeatures{Ligatures=TeX}

% Use fonts that work in Overleaf
...
% Title
\title{%
\textbf{\Huge תיעוד אונטולוגיה לכתבי יד עבריים}\\[1em]
\Large גרסה 1.5 — פברואר 2026\\[0.5em]
\normalsize מבוסס על העיצוב המקורי של פרופ' גילה פרבור\\
עם הרחבות בעקבות משוב מומחים ואינטגרציה עם מזל
}

\author{%
אלכסנדר גולדברג\\
\small בשיתוף: פרופ' גילה פרבור, פרופ' משה לביא, ד"ר אליעזר באומגרטן
}
...
\begin{itemize}
\item \textbf{CIDOC-CRM} — מודל ייחוס מושגי למוזיאונים \en{(ISO 21127)}
...
\textbf{הגישה העיקרית היא אירועית} — תיעוד מה קרה לכתב היד לאורך הזמן (יצירה, העברות בעלות, תיעוד), מהיצירה המופשטת \en{(Work)} לביטוי הטקסטואלי \en{(Expression)} למופע הפיזי \en{(Manifestation/Item)}.
...
\subsection{מבנה תלת-שכבתי \en{(BU-CU-PU)}}

המבנה הפיזי של כתבי יד מיוצג בשלוש רמות:

\begin{itemize}
\item \textbf{BU} \en{(Bibliographic Unit)} — יחידה ביבליוגרפית: הכרך הפיזי השלם
\item \textbf{CU} \en{(Codicological Unit)} — יחידה קודיקולוגית: חיבור מובחן שנוצר בזמן/מקום מסוים
\item \textbf{PU} \en{(Paleographical Unit)} — יחידה פלאוגרפית: חלק כתוב ביד כותב ספציפי
\end{itemize}

כל הרמות הן תת-מחלקות של \textenglish{F4\_Manifestation\_Singleton}, מקושרות ביחסי הרכבה \textenglish{R5\_has\_component}.

\subsection{מרחבי שמות \en{(Namespaces)}}
...
\subsection{\new{אינטגרציה עם מאגר הרשויות הלאומי — מזל}}

\new{החל מגרסה 1.4, המערכת משתלבת עם מאגר הרשויות הישראלי מזל \en{(Mazal)} של הספרייה הלאומית.}

\new{מזל הוא קובץ הרשויות הלאומי הישראלי המכיל כ-4.5 מיליון רשומות רשות, המספקות זיהוי ייחודי ואחיד עבור:}
\begin{itemize}
\item \new{אישים — שמות אישיים}
\item \new{מקומות — שמות גיאוגרפיים}
\item \new{גופים — שמות תאגידים וארגונים}
\item \new{יצירות — כותרות אחידות}
\end{itemize}

\new{יתרונות האינטגרציה:}
\begin{itemize}
\item \new{זיהוי חד-משמעי של ישויות — במקום URI מקומי, משתמשים ב-NLI ID רשמי}
\item \new{קישוריות בינלאומית — מזל מקושר ל-VIAF, Wikidata, ORCID}
\item \new{תמיכה רב-לשונית — עברית, לטינית, ערבית, קירילית}
\item \new{עקביות קטלוגית — שימוש באותה צורת שם כמו בקטלוגים אחרים}
\end{itemize}

\new{דוגמה — לפני ואחרי האינטגרציה:}
...
%--------------------------------------------------------------------
\subsection{מחלקות כלליות \en{(OWL/RDF)}}
%--------------------------------------------------------------------
...
%====================================================================
\section{תכונות אובייקט \en{(Object Properties)}}
%====================================================================
...
\hline
lrmoo:R4i\_is\_embodied\_in & מקשר Expression לגילום \en{(F3/F4 Manifestation)} שבו הוא מגולם.\\
\hline
lrmoo:R4\_embodies & מקשר Manifestation ל-Expression שהוא מגלם.\\
\hline
lrmoo:R7\_exemplifies & עותק יחידאי מגלם מהדורה כללית.\\
\hline
lrmoo:R10\_has\_member & בת-יצירה כוללת יצירות: \en{Work} כולל \en{Work}.\\
\hline
...
\hline
\new{hm:has\_date\_format} & \new{סוג פורמט התאריך \en{(GregorianYear, HebrewYear, FullDate, UnstructuredDate)}.}\\
\hline
...
hm:is\_scribe\_mentioned\_in & יחס הפוך: סופר $\rightarrow$ קולופון שבו הוזכר.\\
\hline
...
\newprop{hm:tradition\_includes} & \new{מסורת כוללת ביטוי (הפוך של \en{belongs\_to\_tradition}).}\\
\hline
...
\hline
\newprop{hm:has\_linked\_work} & \new{גשר פרדיגמות: קישור ל-\en{Work} (צד ביבליוגרפי).}\\
\hline
\newprop{hm:has\_linked\_tradition} & \new{גשר פרדיגמות: קישור ל-\en{TextTradition} (צד פילולוגי).}\\
\hline
...
\newprop{hm:has\_philological\_witness} & \new{מקשר כתב יד ישירות ל-\textenglish{TransmissionWitness} (עדות פילולוגית).}\\
\hline
...
\newprop{hm:follows\_unit} & \new{יחידה עוקבת (הפוך של \en{precedes\_unit}).}\\
\hline
...
%====================================================================
\section{תכונות נתונים \en{(Datatype Properties)}}
%====================================================================
...
hm:wikidata\_id & מזהה ויקידטה & xsd:string\\
\hline
...
%====================================================================
\section{אוצר מילים מבוקר \en{(Instances)}}
%====================================================================
...
\textbf{סוגים אזוריים \en{(TypeScriptType)}:}
\begin{itemize}
...
\textbf{מצבי כתב \en{(ModeScriptType)}:}
\begin{itemize}
...
\item \new{hm:ComplexHierarchy — היררכיה מורכבת \en{(MTM, Sammelband)}}
...
\begin{itemize}
\item מבצע \en{(P14\_carried\_out\_by)}
\item זמן \en{(P4\_has\_time-span)}
\item מקום \en{(P7\_took\_place\_at)}
\item חפצים מעורבים \en{(P16\_used\_specific\_object)}
\end{itemize}
...
\begin{itemize}
\item מזהים חיצוניים \en{(VIAF, Wikidata, NLI)}
...
%--------------------------------------------------------------------
\subsection{כתב יד מורכב \en{(BU + CUs)}}
%--------------------------------------------------------------------
...
נספח זה משחזר את טבלאות המחלקות והמאפיינים כפי שהופיעו במסמך \textenglish{\LR{``תיעוד אונטולוגיה לכתבי יד עבריים - גרסה מעודכנת (3)''}} מתאריך 26.09.2025.

%--------------------------------------------------------------------
\subsection{מחלקות \en{(Classes)} — כפי שמופיע בתיעוד 26-9-25}
%--------------------------------------------------------------------
...
\hline
\textenglish{lrmoo:F5\_Item} & פריט פיזי \en{(LRMoo)} תואם. & \textenglish{cidoc-crm:E18\_Physical\_Thing} \\
\hline
\end{longtable}
}
...
%--------------------------------------------------------------------
\subsection{יחסי עצמים \en{(Object Properties)} — כפי שמופיע בתיעוד 26-9-25}
%--------------------------------------------------------------------
...
\hline
\textenglish{R5\_has\_component} & \textenglish{F4\_Manifestation\_Singleton} & \textenglish{F3/F4\_Manifestation} & הרכבת חלקים \en{(BU→CU→PU)} \\
\hline
...
%--------------------------------------------------------------------
\subsection{מאפייני נתונים \en{(Data Properties)} — כפי שמופיע בתיעוד 26-9-25}
%--------------------------------------------------------------------
...
\hline
\textenglish{wikidata\_id} & \textenglish{owl:Thing} & \textenglish{xsd:string} & מזהה \textenglish{Wikidata} \\
\hline
...
%====================================================================
\section{\new{גרסה 1.5 — מטא-נתונים מנהליים וגישה \en{(Administrative \& Access Metadata)}}}
%====================================================================

\new{גרסה 1.5 מרחיבה את האונטולוגיה עם תמיכה בשדות מנהליים ובנתוני גישה, המאפשרים ייצוג מלא של כל המידע מרשומות \en{MARC} של הספרייה הלאומית.}
...
\newclass{hm:PhysicalHolding} & \new{אחזקה פיזית של כתב היד במוסד מסוים, כולל מיקום וזמינות. נגזר משדה \en{MARC AVA}.}\\
\hline
\newclass{hm:DigitalAccess} & \new{נקודת גישה דיגיטלית לכתב יד, כולל קישורי צפייה מקוונת ו-\en{IIIF}. נגזר משדה \en{MARC AVE}.}\\
\hline
\newclass{hm:UsageRestriction} & \new{הגבלות שימוש והיתרים לכתב יד, כולל זכויות העתקה ומדיניות שכפול. נגזר משדה \en{MARC 939}.}\\
\hline
\newclass{hm:RightsDetermination} & \new{קביעת מצב זכויות לכתב יד, כולל סיווג נחלת הכלל ונימוק. נגזר משדה \en{MARC 952}.}\\
\hline

\end{longtable}

%--------------------------------------------------------------------
\subsection{\new{תכונות אובייקט חדשות}}
%--------------------------------------------------------------------
...
\newprop{hm:has\_physical\_holding} & \new{מקשר כתב יד לאחזקה פיזית.}\\
\hline
\newprop{hm:has\_digital\_access} & \new{מקשר כתב יד לנקודת גישה דיגיטלית. הופכי: \en{is\_digital\_access\_of}.}\\
\hline
\newprop{hm:has\_usage\_restriction} & \new{מקשר כתב יד למדיניות הגבלת שימוש. הופכי: \en{is\_usage\_restriction\_of}.}\\
\hline
\newprop{hm:has\_rights\_determination} & \new{מקשר כתב יד לקביעת זכויות רשמית. הופכי: \en{is\_rights\_determination\_of}.}\\
\hline
\newprop{hm:has\_related\_material} & \new{מקשר כתב יד לחומרים קשורים. סימטרי. נגזר משדה \en{MARC 544}.}\\
\hline
\newprop{hm:held\_at} & \new{מוסד או ספריית משנה בה מוחזק הפריט.}\\
\hline
\newprop{hm:former\_owner} & \new{בעלים קודם של כתב היד. תחום: \en{F4\_Manifestation\_Singleton}, טווח: \en{E21\_Person}.}\\
\hline
\newprop{hm:is\_part\_of\_host} & \new{כתב יד כחלק מפריט מארח. נגזר מ-\en{MARC 773}.}\\
\hline

\end{longtable}

%--------------------------------------------------------------------
\subsection{\new{תכונות נתונים חדשות}}
%--------------------------------------------------------------------

\begin{longtable}{|r|p{4cm}|p{2cm}|p{3cm}|}
\hline
\textbf{תכונה} & \textbf{תיאור} & \textbf{סוג} & \textbf{מקור \en{MARC}} \\
\hline
\endhead

\newprop{hm:copyright\_notice} & \new{הודעת זכויות יוצרים} & \en{xsd:string} & \en{597} \\
\hline
\newprop{hm:related\_material\_note} & \new{הערת חומר קשור} & \en{xsd:string} & \en{544} \\
\hline
\newprop{hm:acquisition\_source} & \new{מקור רכישה} & \en{xsd:string} & \en{541} \\
\hline
\newprop{hm:collection\_name} & \new{שם אוסף} & \en{xsd:string} & \en{966} \\
\hline
\newprop{hm:restriction\_type} & \new{סוג הגבלה} & \en{xsd:string} & \en{939\$a} \\
\hline
\newprop{hm:restriction\_url} & \new{קישור למדיניות הגבלה} & \en{xsd:anyURI} & \en{939\$u} \\
\hline
\newprop{hm:rights\_status} & \new{מצב זכויות} & \en{xsd:string} & \en{952\$a} \\
\hline
\newprop{hm:rights\_basis} & \new{בסיס קביעת זכויות} & \en{xsd:string} & \en{952\$b} \\
\hline
\newprop{hm:holding\_institution} & \new{מוסד מחזיק} & \en{xsd:string} & \en{AVA\$b} \\
\hline
\newprop{hm:holding\_sublibrary} & \new{ספריית משנה} & \en{xsd:string} & \en{AVA\$q} \\
\hline
\newprop{hm:holding\_collection} & \new{אוסף} & \en{xsd:string} & \en{AVA\$c} \\
\hline
\newprop{hm:holding\_call\_number} & \new{מספר קריאה} & \en{xsd:string} & \en{AVA\$d} \\
\hline
\newprop{hm:holding\_availability} & \new{מצב זמינות} & \en{xsd:string} & \en{AVA\$e} \\
\hline
\newprop{hm:digital\_access\_url} & \new{קישור גישה דיגיטלית} & \en{xsd:anyURI} & \en{AVE} \\
\hline
\newprop{hm:digital\_access\_note} & \new{הערת גישה דיגיטלית} & \en{xsd:string} & \en{AVE\$n} \\
\hline
\newprop{hm:digital\_access\_type} & \new{סוג גישה דיגיטלית} & \en{xsd:string} & \en{AVE} \\
\hline
\newprop{hm:shelfmark} & \new{סימן מדף / מספר מערכת} & \en{xsd:string} & \en{942\$z} \\
\hline
\newprop{hm:has\_multiple\_hands} & \new{כתוב בכמה ידיים (סופרים)} & \en{xsd:boolean} & \en{—} \\
\hline
\newprop{hm:anthology\_position} & \new{מיקום מספרי של ביטוי באנתולוגיה; תאימות המרה. ראה להלן מודל אנתולוגיה.} & \en{xsd:integer} & \en{—} \\
\hline

\end{longtable}

%--------------------------------------------------------------------
\subsection{\new{מודל אנתולוגיה — הבהרה}}
%--------------------------------------------------------------------

\new{שלוש תכונות קשורות לאנתולוגיה:}
\begin{itemize}
\item \newprop{hm:anthology\_position} — \new{תכונת נתונים: מספר שלם על \en{F2\_Expression}; מתאימה להמרת \en{MARC} פשוטה.}
\item \newprop{hm:has\_anthology\_position} — \new{תכונת אובייקט: מקשרת ביטוי ל-\en{AnthologyPosition} (אובייקט עם מטא-נתונים).}
\item \newprop{hm:anthology\_order} — \new{תכונת נתונים על \en{AnthologyPosition}: מספר סדר (\en{xsd:integer}).}
\end{itemize}
\new{למודל מלא: צור \en{AnthologyPosition}, השתמש ב-\en{has\_anthology\_position} ו-\en{anthology\_order}. להמרה פשוטה ניתן להשתמש ב-\en{anthology\_position} על הביטוי.}
