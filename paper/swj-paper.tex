...
\begin{abstract}
Transforming library catalogs into Linked Open Data is essential for connecting isolated cultural heritage collections to the Semantic Web, yet automated conversion from MARC~21 records to Wikidata remains a largely unsolved problem---particularly for non-Latin-script collections where existing tools achieve low field coverage. We present the MHM (Mapping Hebrew Manuscripts) Pipeline, a multi-strategy system that turns Hebrew manuscript MARC records into reviewed, published Linked Open Data through three complementary contributions: \emph{authority enrichment as the primary engine of new information}, \emph{publication in three Linked-Open-Data layers}, and \emph{a curator-facing workbench that keeps researchers in the loop}. In the browser deployment these rest on four HMO Wikibase pillars: (1)~live project-Wikibase entities are the post-upload root for RDF and Wikidata projections; (2)~every public Wikidata P/Q is reached through an ontology-backed mapper; (3)~Wikibase schema and instance claims mirror the Hebrew Manuscripts Ontology one-to-one; and (4)~entities are enriched as richly as Mazal, KIMA, VIAF and Wikidata allow under fail-closed matching.

The authority layer feeds \emph{thirteen MARC tags} (100/110/111, 600/610, 651, 700/710/711, 752, 800/810/811) into four complementary authority sources---Mazal/NLI, VIAF, Wikidata, and KIMA---and amplifies each \emph{accepted} match through \emph{VIAF cluster harvesting}, which turns a single VIAF hit into 2--4 additional international identifiers (GND, LCCN, ISNI, BnF), and \emph{Wikidata date back-fill}, which retrieves P569/P570 for SPARQL-only matches so the date guardian can vet them. Matching is deliberately fail-closed: kind-routed lookup, Mazal homonym abstention, KIMA multi-QID abstention, and cross-source VIAF/Wikidata disagreement withhold ambiguous candidates, while every unambiguous hit still mints the richest safe identifier surface (Mazal, KIMA, VIAF cluster, Wikidata, GeoNames) into the HMO graph. The publication layer emits the same data in three forms: the full HMO research graph as RDF, a project Wikibase (\texttt{mhm-hmo.wikibase.cloud}) populated through a guarded live upload path (SHACL-gated, reconciled against existing items before any create) for browsing and editing, and a public Wikidata projection that reaches 87\% of HMO classes and averages 30.7 statements per manuscript---in the browser port restricted to manuscript/person/work items with summarized HMO nodes rolled onto parents and the full MARC/authority claim surface merged onto those HMO-rooted items. The workbench layer, \emph{Wikidata Studio}, surfaces every authority match, NER entity and KIMA place under a Source~$\times$~Type~$\times$~Role filter, opens a biographic-comparison dialog that aligns MARC, Mazal, VIAF and KIMA biodata side-by-side, exposes a confidence tooltip that explains why each match scored as it did, runs 14 pre-upload guards, and saves resumable review state---so every published statement is one a human approved with the evidence in view.
...
\item \textbf{MARC-to-LOD publication in three coordinated layers.} The same pipeline emits the full HMO research graph as RDF, publishes it to a project Wikibase at \texttt{mhm-hmo.wikibase.cloud} through a guarded live upload path---every item is SHACL-gated and reconciled against the existing Wikibase before any create---for browsing and curator editing, and projects a public-facing view onto Wikidata that reaches 87\% of HMO classes and averages 30.7 statements per manuscript. In the browser port the project Wikibase is the scholarly root after read-back: RDF and Wikidata Studio consume durable canonical entities; the ontology file is the 1:1 schema contract; and a dedicated HMO$\to$Wikidata P/Q mapper (never project-ID identity) bridges the two namespaces. Public Wikidata Studio items are restricted to the WikiProject Manuscripts entity types (\emph{manuscript}, \emph{person}, \emph{work}); HMO classes marked \texttt{summarized\_in\_wikidata} roll allowlisted claims onto the parent item rather than minting ontology-class QIDs, and the canonical Studio build merges the full MARC/authority claim surface onto those HMO-rooted items so research properties (production, language, contents, housing, provenance, occupations) are not lost to identity shells. To our knowledge this is the first time the NLI Hebrew manuscript catalog has been linked, end-to-end, to the public Semantic Web.
\item \textbf{Wikidata Studio: a curator-facing workbench that puts the researcher in control.} The workbench ships in both a cross-platform PyQt6 desktop application and a collaborative browser-based port (\texttt{mhm-pipeline-web}). Both surfaces expose every authority match, NER entity and KIMA place under a Source~$\times$~Type~$\times$~Role filter; a biographic-comparison dialog aligns MARC, Mazal, VIAF and KIMA biodata side-by-side (with Unicode NFKC normalisation, particle stripping, and side-aware highlighting); per-item override dialogs for Wikidata labels, descriptions, aliases and statements; a confidence tooltip explains which guardians fired and which cluster identifiers came along with the match; 14 pre-upload guards block malformed items (17 in the browser port, which adds three property-audit checks); and review state is persisted so curators can pause and resume. The browser port adds shared multi-curator approval, a persistent versioning log, and encrypted per-user API keys. When the eval-agent produces a high-confidence suggested fix for a NER entity, an \textbf{Auto-fix} button appears on the relevant table row and in the detail drawer, letting the curator apply the correction in one click without pre-approving the entity; the same pattern extends to project-Wikibase items, where an \emph{Apply AI fix} / \emph{Apply fix \& push} pair lets the curator accept a verified correction and push the single corrected item live without a full re-upload. This is what makes every published statement a human-approved one and is what turns the pipeline into a research tool rather than a script.
...
\item The first \textbf{end-to-end quantitative evaluation} of MARC-to-Wikidata conversion that measures both coverage (statements per entity) and data loss (what cannot transfer and why), together with a \textbf{five-category taxonomy of data loss} showing that the dominant remaining loss is structural---work-title strings must be reconciled to or created as Wikidata items before P1574 can be asserted---rather than technical. Within that taxonomy we further separate \emph{parser} loss from \emph{knowledge} loss: on the pinned subset, fields present in MARC but unprojected (856\$u digital access on 63 of 68 records, multi-sequence and Hebrew-numeral extents in 300\$a) were recovered deterministically, while narrative provenance prose in 500/541/561 is handled by span-grounded LLM extraction whose output is a curator proposal carrying its own verbatim textual warrant, never an auto-projected claim (Strategy~I).
...
\item \textbf{Authority Resolution}: Matches names against four authority sources: Mazal/NLI (local SQLite, $\sim$2.5~million records), VIAF (international, SRU API), KIMA (Hebrew place gazetteer), and Wikidata (SPARQL, public). Lookups are kind-routed (person matchers never fire on places or works; places run KIMA then Mazal place; works run Mazal work only) and fail-closed on ambiguity (Mazal personality homonym abstention; KIMA multi-QID abstention). The set of MARC name- and place-bearing fields consulted at this stage was expanded in v1.10 (\texttt{CLAUDE.md}~rule~49) from the six original tags (100/110/111 main entry, 700/710/711 added entry) to thirteen: \emph{subject} headings 600/610/651, \emph{hierarchical place name} 752, and \emph{series added entries} 800/810/811 now feed the matcher alongside the original six. Subject persons (MARC~600) take a dedicated \texttt{role="subject"} that suppresses the death-side date guard --- a manuscript can be \emph{about} someone who died centuries before its production. VIAF cluster harvesting extracts cross-referenced GND, LCCN, ISNI, and BnF identifiers that later become HMO \texttt{owl:sameAs} links.
...
\item \textbf{Wikidata Projection and Upload}: Projects the validated HMO graph into WikibaseIntegrator item representations and uploads or exports them via the Wikidata API / QuickStatements with OAuth~2.0 authentication, claim-level diffing (no duplicate statements), and batch rate limiting. The same stage also emits a projection-coverage report and a project-Wikibase export covering the full HMO graph: each typed HMO node becomes a project-Wikibase entity and RDF statements are preserved as local statements, while Wikidata receives only the conservative public projection. In the browser deployment this export feeds a live, guarded upload to \texttt{mhm-hmo.wikibase.cloud} (\S\ref{sec:deployment}); the desktop application retains the offline draft files.
\end{enumerate}

Every stage exposes a substep telemetry channel (\texttt{substep~=~pyqtSignal(str)} on the \texttt{StageWorker} base class) so the GUI's unified \texttt{DynamicProgressBar} surfaces the running operation (e.g.\ ``Stage 3.1~--~Mazal lookup ($i/n$)'') alongside percentage and rolling-window ETA. All nine workers --- the six pipeline stages plus the Mazal and KIMA authority-index builders and the eval-agent runner --- emit through the same channel, so the user sees one consistent progress convention regardless of which stage is running.
...
\paragraph{Desktop application.} A cross-platform PyQt6 executable (macOS DMG, Windows installer) bundles the four model checkpoints (~2.9\,GB total; the joint person-NER model ships as a \texttt{safetensors} directory, the other three as \texttt{.pt} files) and a DictaBERT base alongside the application. Stage~2 inference runs in-process via PyTorch with MPS$\rightarrow$CUDA$\rightarrow$CPU device fallthrough. This is the canonical research deployment: full offline operation, no per-call cost, and the lowest latency (~50\,ms per record on Apple Silicon). The bundled-models lockstep contract is enforced by a cross-platform parity test that fails the build if either installer drops a checkpoint.

\paragraph{Collaborative web port.} The same pipeline backend is exposed as a FastAPI service with a React/TypeScript frontend at \texttt{mhm-pipeline-web} (project repository), deployable to standard PaaS environments. The browser surface mirrors the desktop's curator workflow: NER review with agentic AI verification, HMO Wikibase item review, Wikidata Studio review, and a shared bidirectional approval store so multiple curators can review the same run live. Standalone Authority mutation routes are retired by default (HTTP~410); live enrichment runs inside HMO entity creation (\texttt{refresh\_authority=true}), which re-matches Mazal/KIMA/VIAF/Wikidata, rebuilds the RDF TTL from approved matches, persists a durable \texttt{RdfArtifact}, and only then exports Wikibase items---so refreshed identifiers cannot remain trapped behind a stale graph. Every curator surface supports per-field manual override---NER entities (\texttt{override\_text/type/role}), Wikidata Studio item labels/descriptions/statements, HMO item overrides, and raw MARC JSON---with a per-entity versioning log (RFC~6902 patches) and project export bundles so research teams can audit and replay decisions. Encrypted per-user secrets (Gemini key, Wikidata token, Wikibase Cloud bot password) are stored under a password-derived KEK (zero-knowledge recovery). In this deployment the Mazal and KIMA authority databases are imported into the same Postgres instance that backs the application (\texttt{AUTHORITY\_MODE=postgres}), so authority resolution needs no local SQLite files; Postgres KIMA lookup uses the same multi-row Wikidata-QID abstention as the desktop SQLite index rather than an arbitrary first hit. The browser port also operates the project-Wikibase layer live: an \emph{HMO Wikibase Studio} bootstraps the Wikibase schema from the ontology, builds an item for every typed node in a run's HMO graph, blocks any item carrying SHACL \emph{Violation}/\emph{Error} findings, reconciles each item against the live Wikibase by its HMO source URI before any create --- adopting a matching existing item instead of duplicating it, and failing closed when the lookup itself fails --- records every write in an audit log, supports single-item pushes so a curator-applied fix reaches the wiki without a full corpus re-upload, and treats complete live read-back into durable \texttt{hmo\_canonical\_entities} as the source of truth for downstream RDF/Wikidata projections. Wikidata Studio on the same deployment defaults to that canonical source: it emits only public manuscript/person/work items, folds summarized HMO nodes (production events, codicological units, expressions, and related classes) onto those parents through the ontology P/Q mapper, then merges the legacy MARC/authority builder's research claims onto the HMO-rooted items by control number, VIAF/Mazal identity, or work title---preserving HMO \texttt{local\_id}s and project-Wikibase bridges (P2888/P973) while restoring production dates (P571), places (P1071/P7153), language (P407), contents (P1574), housing (P195/P217), material and folios, provenance owners, person occupations (P106), and work authors (P50/P2093). Heavy Studio builds, uploads, and AI-verify jobs run as claimed background workers that never SPARQL-reconcile the whole corpus on the build/verify path (live WDQS reconcile stays on upload and gated QuickStatements); AI verify attaches a multi-channel evidence pack (MARC, VIAF, Mazal/NLI, existing Wikidata, and HMO Wikibase) plus a WikiProject Manuscripts skill slice so the judge sees the same evidence the builder used.

\paragraph{Pay-per-call inference backend.} To support web deployments without bundling model weights into the application slug, all four Stage~2 models (joint Person NER, Provenance NER, Contents NER, Genre Classifier) are also published as public Hugging Face Hub repositories\footnote{\url{https://huggingface.co/alexgoldberg/hebrew-manuscript-joint-ner-v2}, \url{https://huggingface.co/alexgoldberg/hebrew-manuscript-provenance-ner}, \url{https://huggingface.co/alexgoldberg/hebrew-manuscript-contents-ner}, \url{https://huggingface.co/alexgoldberg/hebrew-manuscript-genre-classifier}.} and bundled together in a single serverless container hosted on Modal.\footnote{\url{https://modal.com/}.} A single \texttt{MhmNer} class loads all four pipelines into one container at warm-up (sharing the DictaBERT encoder across the three derived NER models), exposes a single \texttt{POST /extract} endpoint that returns all four model outputs in one round-trip per record, and scales to zero after 5~minutes of inactivity. Cold start: ~30--60\,s for the four-model load; warm latency: ~300\,ms per record on a 2-CPU container. Billed per-second of container time, this works out to approximately \$0.00006 per record (well under \$1 for a 10{,}000-record corpus). Modal's free-tier credit covers typical research workloads entirely. The web FastAPI backend selects between \emph{local} (in-process torch), \emph{HuggingFace Inference Providers}, and \emph{Modal} extraction backends through an \texttt{EXTRACTION\_MODE} environment variable; the choice is transparent to the rest of the pipeline. A shared cross-user inference cache routes every external NER, classifier, and authority call through a two-tier lookup: a Redis L1 tier (sub-millisecond, content-addressed by SHA-256 of canonical query JSON) backed by a Postgres L2 tier (durable; per-source authority TTLs of 90 days for Mazal, 30 days for VIAF and Wikidata, and 180 days for KIMA behind a 24-hour Redis hot window; 90 days for AI-judge verdicts; no expiry for NER/classifier outputs). The first team member to resolve a candidate populates both tiers; every subsequent request warm-hits Redis without touching Postgres.
...
We created comprehensive mapping dictionaries: 50~genre terms to Wikidata QIDs (covering 100\% of NLI's genre vocabulary), 47~LCSH subject headings, 13~Bible books, and 14~Talmud Bavli tractates. All mappings are centralized in a single module (\texttt{property\_mapping.py}) for maintainability.

\subsection{Strategy C: Multi-Authority Linking}

Four authority sources provide entity resolution:

\textbf{Mazal/NLI} ($\sim$2.5~million records; a local SQLite index on the desktop, imported into Postgres in the browser deployment): Fuzzy matching of person, place, work, and corporate body names against the NLI authority file. Provides NLI~J9U identifiers (P8189) and biographical dates.
...
\textbf{Homonym abstention and richness without guessing.} When multiple Mazal personality rows share a normalised name and MARC~\texttt{\$d} dates do not disambiguate, \texttt{pick\_mazal\_candidate} scores candidates (tag-100 preference, date overlap, manuscript-year plausibility) and abstains when the top two scores are within a tight margin without date evidence; person VIAF/Wikidata searches are suppressed on abstain so the curator can pick from scored candidates rather than inherit a guessed QID. The complementary design goal is richness under that fail-closed boundary: every accepted match still carries the full payload contract (preferred Hebrew/Latin names, Mazal NLI id, VIAF URI, Wikidata QID, VIAF \texttt{cluster\_ids}, KIMA geo slice), and GraphBuilder mints the corresponding HMO claims---\texttt{hm:mazal\_id}, \texttt{hm:viaf\_id}, \texttt{hm:wikidata\_id}, cluster \texttt{owl:sameAs} URIs, place \texttt{hm:kima\_id}/\texttt{hm:geonames\_id}---so project-Wikibase entities are as internationally linked as the evidence safely allows. Ambiguous candidates remain in review evidence with \texttt{accepted: false}; they never become public claims.
...
\begin{enumerate}
\item \textbf{Curated override dictionary} (deterministic, $\sim$44 entries): canonical English forms for famous figures and works \textemdash{} \textit{Maimonides}, \textit{Rashi}, \textit{Nahmanides}, \textit{Joseph Karo}, \textit{Ibn Ezra}, \textit{Mishnah}, \textit{Talmud}, etc.
...
\subsection{Strategy I: Span-Grounded LLM Extraction During Entity Creation}
\label{sec:llm-extraction}

Strategies A--H recover metadata that MARC states in a structured or patterned way. A residual class resists them entirely: provenance recorded as narrative Hebrew prose in fields 500, 541, 561, 563 and 583. A representative 561\$a reads

\begin{quote}
\small
\code{בדף 241א רשימת הבעלים "אברהם היכיני" המזכירה את נשואי הוריו בשנת "שלש מאות ושבעים ושש לבריאת עולם"}
\end{quote}

\noindent{}---\,``on leaf 241a, an owners' list \textit{Abraham ha-Yakhini}, mentioning his parents' marriage in the year 376 of creation''. This states an owner, a place and a Hebrew-calendar date in running text. On the pinned 68-record subset, 37~records carry such prose and none yielded a single owner or location claim under the pattern-based strategies.

\paragraph{Recovery precedes generation.}
Before introducing a generative component we measured how much of the apparent gap was in fact prose at all. It was mostly not. Field 856\$u (digital access) was present on 63 of 68 records while only one item carried P953, because the collapsed-key web ingest never derived the URL; field 300\$a was present on 62 records with P1104 projected on 45, because the extent parser took the first number adjacent to a folio unit and therefore mis-stated multi-sequence extents (\code{111, [2] דף} became 2 rather than 113) and silently dropped page-unit, volume-collated and Hebrew-numeral (gematria) extents such as \code{ריג דף} (213). These are parser defects, not knowledge gaps, and we fixed them deterministically: a single extent parser now sums every sequence, resolves the unit explicitly, and returns \emph{no value} for a unit it cannot name (\code{עמודות}, columns), for a folio \emph{reference} rather than a count (\code{דף 2א-2ב}), and for a bare number. This ordering is a design rule of the system: an LLM is used only where no parser can reach, never as a substitute for reading the record correctly.

\paragraph{Span grounding.}
For the prose residue we prompt a tier-1 instruction model (DeepSeek~V4~Flash, served through Qubrid) with the note fields and a closed list of target properties, and require every proposal to quote the verbatim substring that states it. A proposal whose span is not literally present in the record is discarded before it reaches a curator. This converts the model's role from \emph{generation} to \emph{grounded extraction}: it may locate and structure an assertion the cataloger made, but it cannot introduce one. Two further constraints keep the output inside the audited ontology: the property must be one of the already-verified constants (P127 owner, P1071 location of creation, P186 material), so extraction can never mint a new property or item identifier, and material values must resolve through the same closed vocabulary the deterministic path uses.

\paragraph{Proposals are not claims.}
Extraction output is never projected into an item's statements. It is attached as a distinct evidence channel that the curator interface and the agentic verifier both display, and the verifier's rubric explicitly forbids a proposal from supporting a claim or from influencing any verdict axis. This follows from the system's evidence discipline: each public claim must name the catalog channel that supports it, and a model's paraphrase is not such a channel. The design consequence is deliberate---a false proposal costs a curator one rejected suggestion, whereas an auto-projected false claim would enter the public graph. Results are content-addressed on the record text and cached, so re-running a build is free, and the component is bounded by a per-build call budget and disabled by a single environment flag.

\paragraph{What this buys.}
The distinction matters for the coverage taxonomy of \nameref{sec:data-loss}. Prose provenance previously fell into the ``text available, no structured value'' category with no path forward other than manual reading. Span-grounded extraction converts it into a reviewable queue: the curator confirms or rejects a candidate that already carries its own textual warrant and MARC tag, which is a substantially cheaper operation than locating the assertion in the record unaided.
...
These barriers motivate a three-layer publication strategy. The HMO RDF graph remains the canonical scholarly representation; the project Wikibase at \texttt{mhm-hmo.wikibase.cloud} exposes the same HMO-level entities in a browsable and editable knowledge-base interface; and Wikidata receives the richest safe public projection for broad discovery and cross-collection linking. The projection is therefore explicit rather than silent: each HMO class is classified as directly projected to Wikidata, summarized in Wikidata, or retained only in HMO / project Wikibase. Summarized classes contribute claims to the parent public item rather than appearing as ontology-class QIDs on Wikidata; in the browser deployment the canonical Studio path further merges MARC/authority research claims onto those HMO-rooted items so the public layer stays both WikiProject-Manuscripts-safe and catalog-complete. The project-Wikibase layer is not a looser duplicate of Wikidata; it is a controlled deployment of the ontology-native graph, intended to retain HMO entities and relationships that public Wikidata should not flatten or over-model.
...
Each issue was fixed and protected by a regression test. The full reconciler now checks all five identifier types before creating any new person entity; the institutional name detector routes MARC~710 entries to the appropriate collection/ownership property; P8189 is gated on both authority-record prefix and \texttt{P31=Q5} (human); and placeholder titles are detected and replaced with shelfmark-based labels. Fourteen pre-upload validation checks in \texttt{item\_validator.py} block any item carrying a known failure pattern (empty label, anonymous person, kovetz placeholder, trailing punctuation, MARC-inverted label, institutional-as-person, P3959 on human, P8189 bad prefix, VIAF on non-person, multiple same-language P1559, no-identifier person, P1559 Latin-as-Hebrew, Hebrew label is Latin, ambiguous single name) from being approved in the GUI; every error-level finding disables the ``Approved'' checkbox for that row. The browser port extends the same validator with three property-audit checks added after a live audit of every P/Q constant in the mapping module (P50 on a manuscript --- authorship must run through the P1574 exemplar chain; P7416 as a quantity --- P7416 is a folio citation qualifier, not a count property; a known-bad P31 class QID), bringing the count there to seventeen, and enforces the validator as a hard gate inside the upload path itself: reconciliation against existing items happens \emph{inside} the write path (manuscripts by P3959, persons by cross-checked identifiers, works by label and author), a failed reconciliation lookup blocks the item rather than being read as ``no existing item'', and any error-level validator finding blocks the write regardless of curator approval state. Public Wikidata writes further require an alive-QID confirmation and an own-or-accept ownership check: the bot may create after dedup, update items it first created, or touch a foreign existing item only when the curator has explicitly accepted that exact QID. Project-Wikibase properties and classes map to public Wikidata P/Q identifiers through ontology \texttt{owl:equivalentProperty}/\texttt{equivalentClass} and an explicit allowlist---never by treating a local project QID as a Wikidata QID.
...
The 501~unit tests in the safety/regression suite (\texttt{test\_safety\_guards.py}; 823 across the full unit suite) serve as a barrier ensuring these classes of error cannot recur. Three of those tests are \emph{structural} rather than behavioural --- they fail if a future refactor introduces a second \code{wbi\_item.write()} call, separates the pre-write guard from the write, or re-introduces a marker-based creator fallback --- so the architecture of Rule~38 cannot be unintentionally weakened.
...
\textbf{RDF-first source and projection audit.} The workbench now treats \texttt{output.ttl} as the preferred Stage~6 input. When a reviewed authority file exists beside it, the projection reuses that sidecar only for curator-approved authority decisions; the HMO RDF graph remains the source being projected. Every RDF-based run emits three audit artefacts: \texttt{wikidata\_source\_report.json} records whether the source was raw authority data, reviewed authority data, HMO RDF, or a saved Wikidata-review state; \texttt{wikidata\_projection\_coverage.json} reports how each HMO class is represented or intentionally not represented in Wikidata; and \texttt{wikibase\_entities.json} / \texttt{wikibase\_quickstatements.txt} provide an offline project-Wikibase draft for the full HMO graph. The project-Wikibase export is complete at the entity/statement level relative to the generated HMO graph: every typed HMO node is represented as an entity, and all non-type/non-label RDF assertions are carried as statements. In the browser port this layer is deployed live: the HMO Wikibase Studio bootstraps the Wikibase schema from the ontology itself, refreshes Mazal/KIMA/VIAF/Wikidata evidence as part of every item build (rebuilding RDF so enrichment reaches the TTL), passes every built item through an export-quality gate and a SHACL gate before upload, reconciles each item against the live Wikibase by its HMO source URI before any create (a matching existing item is adopted rather than duplicated), and records every write in an audit log. Wikidata Studio's canonical mode then builds the public manuscript/person/work review table from those durable entities, rolls summarized HMO nodes onto parents, and merges the MARC/authority claim surface so production, contents, housing, and provenance statements survive the HMO$\to$Wikidata hop. Multi-typed RDF resources still expose one preferred class per item; an explicit list of all RDF classes remains a planned schema-fidelity refinement.
...
\item \textbf{LLM extraction recall is unmeasured}: the span-grounding guard (Strategy~I) bounds \emph{precision} by construction---a proposal whose quoted span is absent from the record cannot reach a curator---but we have not measured recall against a hand-annotated provenance gold standard, so we do not claim how much of the prose residue it finds. Its output is also deliberately advisory: proposals are surfaced for curator confirmation and are barred from supporting a claim or influencing an automated verdict, so the component improves review throughput rather than coverage on its own.
...
We presented the MHM Pipeline as three contributions that share a single processing pipeline. \emph{First}, authority enrichment across thirteen MARC tags (100/110/111, 600/610, 651, 700/710/711, 752, 800/810/811) against four complementary sources (Mazal/NLI, VIAF, Wikidata, KIMA), amplified by VIAF cluster harvesting and Wikidata date back-fill, is the primary engine of new information per manuscript---it turns every \emph{accepted} match from a string into a linked, internationally-identified item and multiplies the international identifier count $1.7\times$ per matched person, while fail-closed matching (kind routing, Mazal homonym abstention, KIMA multi-QID abstention, cross-source VIAF/Wikidata disagreement) keeps ambiguous candidates out of the public graph. \emph{Second}, the same pipeline publishes its output in three coordinated Linked-Open-Data layers: the full HMO research graph as RDF, a project Wikibase at \texttt{mhm-hmo.wikibase.cloud} populated through a guarded live upload path (SHACL-gated, reconciled before any create) for browsing and curator editing, and a public Wikidata projection that reaches 87\% of HMO classes and averages 30.7~statements per manuscript on the pinned 68-record verification subset---with the browser port keeping only manuscript/person/work public items, rolling summarized HMO nodes onto those parents, and merging MARC/authority research claims so the public layer is not an HMO identity shell. \emph{Third}, Wikidata Studio puts a researcher-facing workbench around the whole flow: a Source~$\times$~Type~$\times$~Role review filter, a side-by-side biographic-comparison dialog, a confidence tooltip that surfaces every guardian and cluster identifier behind a match, 14 pre-upload guards, and resumable JSON review state---so every statement that reaches Wikidata is one a human approved with the evidence in view.
...
Five directions are planned: (1)~expanding work-item reconciliation beyond the 114 automatically created subset items so more P1574 claims can be confirmed rather than qualified; (2)~evaluating the pipeline on the full 123,621-record NLI catalog; (3)~adapting the distant supervision methodology to Arabic and Latin manuscript collections to test cross-lingual transferability; (4)~replacing the \emph{presumably} qualifier on NER-identified works with confirmed Wikidata reconciliation once the work-item corpus is established; and (5)~scaling the now-live project-Wikibase deployment from per-run corpora to the full catalog, including explicit preservation of multi-class RDF typing in the project-Wikibase schema, so codicological units, textual witnesses, text traditions, and epistemological structures can be reviewed and queried without forcing them into Wikidata's public property model.
