...
\textbf{Architecture.} We use a frozen \texttt{dicta-il/dictabert} encoder as a feature extractor. The [CLS] token representation is passed through dropout (0.3) and a single linear layer to produce logits over the 14 most frequent genre classes (those appearing in $\geq$5 training records). Binary cross-entropy with logistic loss (\texttt{BCEWithLogitsLoss}) with per-class positive weights $w_c = n_{\text{neg},c} / n_{\text{pos},c}$ corrects for the severe class imbalance inherent in this label set. Freezing the encoder prevents overfitting on the 347 available training samples.

\textbf{Training.} We apply 5-fold stratified cross-validation, saving the best-fold checkpoint. Training runs for up to 30 epochs with early stopping (patience = 5), AdamW optimizer (lr = $10^{-4}$, weight decay = 0.01), and linear warmup over 10\% of steps. The best fold achieves micro-F1 = 0.39 on held-out data; the 5-fold mean is $0.35 \pm 0.02$.
