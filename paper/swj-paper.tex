...
The pipeline maps to 30+ Wikidata properties following the WikiProject Manuscripts data model. Table~\ref{tab:manuscript-props} shows manuscript properties; Table~\ref{tab:person-props} shows person properties.
...
P921 & main subject     & MARC 630   & 46\%  \\
P1071& location created & KIMA       & 36\%  \\
P50  & author           & NER+Auth   & 100\% \\
P11603&transcribed by   & NER+Auth   & 19\%  \\
P127 & owned by         & Prov.\ NER & 20\%  \\
...
\midrule
\multicolumn{3}{l}{\textbf{Average statements per person}} & \textbf{6.4} \\
\bottomrule
\end{tabular}
\end{table}
...
A critical contribution of this work is the systematic quantification of data loss across the MARC-to-Wikidata conversion pipeline. Of 59~fields extracted from MARC records, 30~map to Wikidata properties and 29~do not. Among unmapped fields, 13~contain actual data in our test set. We identify five categories of data loss (Table~\ref{tab:data-loss}).
...
\textbf{2. No Wikidata}\newline property exists
  & colophon\_text, has\_explicit, scribal\_interventions, has\_multiple\_hands, codicological\_units
  & 41, 2, 5, 3, 99
  & Manuscript-specific concepts absent from Wikidata's property model. \\
\addlinespace
...
\textbf{4. Needs further}\newline entity resolution
  & related\_places, 19 subject terms
  & 36, 19
  & Place strings without KIMA QIDs; LCSH terms without QID mappings. \\
\addlinespace
...
\textbf{The dominant loss is Category~1}: 4,169~work titles across 100~manuscripts cannot be mapped because Wikidata's P1574 (exemplar~of) requires item QIDs, and only 12~of the 3,968~unique Hebrew works referenced in these manuscripts have existing Wikidata items. Creating items for these works would require a separate reconciliation effort beyond the scope of automated conversion.

Categories~2 and~3 represent \emph{structural incompatibility} between MARC's document-centric model and Wikidata's entity-centric model. Concepts like colophon text, scribal interventions, and codicological unit structure have no Wikidata representation and may never have one---they are better served by domain-specific ontologies (e.g., our HMO) expressed in full RDF.

Category~4 represents \emph{recoverable loss}: adding QID mappings for 19~subject terms and resolving 36~place names would recover approximately 55~additional claims.
...
P921 (subject)   & 0\%   & \textbf{46\%}& Canonical refs \\
P1071 (location) & 0\%   & \textbf{36\%}& KIMA \\
P127 (owner)     & 0\%   & \textbf{20\%}& Provenance NER \\
P11603 (scribe)  & 0\%   & \textbf{19\%}& Person NER \\
\midrule
\textbf{Avg stmts/MS} & \textbf{$\sim$8} & \textbf{22.9} & Combined \\
\bottomrule
\end{tabular}
\end{table}

The multi-strategy approach yields a 2.9$\times$ improvement over rule-only conversion. Each strategy contributes coverage that the others cannot provide: NER unlocks free-text entity extraction (P50: 1$\to$7.6 authors/MS), rule-based parsing handles Hebrew-specific date formats (P571: 22\%$\to$96\%), and authority linking provides LOD identifiers.
...
\midrule
Full pipeline (v1.9)      & 22.9 & 6.4 \\
$-$ VIAF cluster harvest  & 22.9 & 4.5 \\
$-$ P17/P131 hardcoded    & 20.9 & 6.4 \\
$-$ Provenance NER        & 21.5 & 6.4 \\
$-$ Person NER            & 16.0 & 6.4 \\
$-$ Hebrew date parsing   & 21.5 & 6.4 \\
$-$ Genre QID mapping     & 22.2 & 6.4 \\
Rule-only (no NER/LOD)    & 8.0  & 2.0 \\
\bottomrule
\end{tabular}
\end{table}

Person NER has the largest individual impact ($-$6.9 stmts/MS), followed by Hebrew date parsing ($-$1.4) and provenance NER ($-$1.4). VIAF cluster harvesting has minimal impact on manuscript statement counts but increases person entity richness by 42\% (4.5$\to$6.4 stmts/person).
...
We presented the MHM Pipeline, a multi-strategy system for automated MARC-to-Wikidata conversion that achieves 22.9~statements per manuscript and 6.4~statements per person entity on Hebrew manuscript records from the National Library of Israel. Our five-strategy approach---distant supervision NER, rule-based extraction, multi-authority linking, VIAF cluster harvesting, and property-level validation---demonstrates that no single technique suffices for high-coverage bibliographic LOD conversion.
