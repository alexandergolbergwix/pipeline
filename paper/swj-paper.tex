...
31\% of NLI Hebrew manuscript records have no MARC~655 genre/form headings, leaving P136 (genre) empty after rule-based QID mapping. To fill this gap we apply a fourth form of distant supervision: records that \emph{do} have MARC~655 labels provide (title + notes text, genre label) training pairs for a multi-label sequence classifier.

\textbf{Data.} Training labels come from all 123,621 NLI Hebrew manuscript records — the full Ktiv catalog export. Of these, 31,048 records (25\%) carry at least one MARC~655 genre/form heading that maps to a \texttt{GENRE\_TO\_QID} key; these form the labeled training set. The remaining 75\% are inference targets. The full catalog TSV is augmented with the 897 intensively annotated records used for the NER models (\texttt{17th\_century\_samples.tsv} + \texttt{top100\_richest.tsv}).

\textbf{Architecture.} We use a frozen \texttt{dicta-il/dictabert} encoder as a feature extractor. The [CLS] token representation is passed through dropout (0.3) and a single linear layer to produce logits over 30 output classes: the 29 most frequent genre classes (those appearing in $\geq$50 training records) plus an explicit \emph{None-of-the-Above} (NOTA) class for manuscripts whose genres fall outside this vocabulary. Binary cross-entropy with logistic loss (\texttt{BCEWithLogitsLoss}) with per-class positive weights $w_c = n_{\text{neg},c} / n_{\text{pos},c}$ corrects for the severe class imbalance inherent in this label set. Records whose MARC~655 genres all fall below the frequency threshold contribute as NOTA training examples. Freezing the encoder prevents overfitting relative to training set size.

\textbf{Training.} We apply 5-fold stratified cross-validation, saving the best-fold checkpoint. Training runs for up to 30 epochs with early stopping (patience = 5), AdamW optimizer (lr = $10^{-4}$, weight decay = 0.01), and linear warmup over 10\% of steps. The 29 frequent genre classes include: \emph{Piyyutim} (6,229 examples), \emph{Poetry} (5,581), \emph{Illustrated works} (5,370), \emph{Personal correspondence} (3,581), \emph{Autograph manuscripts} (2,528), \emph{Censored manuscripts} (2,399), \emph{Literature, miscellaneous} (2,338), and 22 additional genre types each with $\geq$50 examples (Family records, Records, Bibliographies, Tales, Community records, Riddles, Legislation, Registers of births, Gittin, Business records, Forms, Biographies, Pinkasim, Parodies, Drama, Licenses, Wills, Death registers, Negotiable instruments, Ketubbot, Mezuzot, Account books).

\textbf{Integration.} At inference time, when a manuscript record contains no MARC~655 genres, the genre classifier is invoked on the concatenated MARC~245 title and first three MARC~500 general notes. Predictions above threshold 0.5 are added as P136 statements with two epistemic qualifiers: P1480 = Q18122778 (\emph{presumably}) and a P887 (\emph{based on heuristic}) reference citing Q2539 (\emph{machine learning}), distinguishing classifier-inferred genres from authority-sourced MARC data. The classifier is loaded lazily and degrades gracefully (no crash, no inferred genres) when the model file is absent.
