...
\textbf{Architecture.} We use a frozen \texttt{dicta-il/dictabert} encoder as a feature extractor. The [CLS] token representation is passed through dropout (0.3) and a single linear layer to produce logits over 8 output classes: the 7 most frequent genre classes (those appearing in $\geq$20 training records) plus an explicit \emph{None-of-the-Above} (NOTA) class for manuscripts whose genres fall outside this vocabulary. Binary cross-entropy with logistic loss (\texttt{BCEWithLogitsLoss}) with per-class positive weights $w_c = n_{\text{neg},c} / n_{\text{pos},c}$ corrects for the severe class imbalance inherent in this label set. The 55 records whose MARC~655 genres all fall below the frequency threshold contribute as NOTA training examples. Freezing the encoder prevents overfitting on the 364 available training samples.

\textbf{Training.} We apply 5-fold stratified cross-validation, saving the best-fold checkpoint. Training runs for up to 30 epochs with early stopping (patience = 5), AdamW optimizer (lr = $10^{-4}$, weight decay = 0.01), and linear warmup over 10\% of steps. The 7 frequent genre classes are: \emph{Illustrated works} (101 examples), \emph{Autograph manuscripts} (82), \emph{Censored manuscripts} (56), \emph{Poetry} (38), \emph{Piyyutim} (35), \emph{Family records} (21), and \emph{Records (Documents)} (20). The best fold achieves micro-F1 = 0.42 on held-out data; the 5-fold mean is $0.37 \pm 0.03$.
