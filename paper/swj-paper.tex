...
\item \textbf{NER Extraction} (3 NER models + 2 sentence-level classifiers): Applies three DictaBERT-based NER models in parallel to different MARC fields:
  ...
  \end{itemize}
  Two complementary classifiers (genre, Strategy~F; MARC~500 sentence classifier, Strategy~G) run alongside the NER models in the same Stage~2 worker, sharing the DictaBERT encoder.

\item \textbf{Authority Resolution}: Matches extracted names against four authority sources: Mazal/NLI (local SQLite, $\sim$2.5~million records), VIAF (international, SRU API), KIMA (Hebrew place gazetteer), and Wikidata (SPARQL, public). VIAF cluster harvesting extracts cross-referenced GND, LCCN, ISNI, and BnF identifiers.
...
P1412 & language       & MARC 008/041    & 332 \\
...
\subsection{Strategy G: MARC 500 Sentence Classifier}
\label{sec:marc500-classifier}

MARC~500 (general notes) is the catch-all field of the MARC record: catalogers use it for colophon attestations, fragments of provenance history, codicological observations, and unrelated commentary. Routing each sentence to the right downstream consumer --- P1684 (inscription) for colophons versus the Provenance NER pipeline for ownership-history sentences versus discarded otherwise --- requires sentence-level classification rather than naive whole-field heuristics.

\textbf{Architecture.} A single DictaBERT encoder feeds two independent sigmoid heads, one per class (COLOPHON, PROVENANCE), trained jointly. The heads share encoder representations but make independent decisions, so a sentence may be flagged simultaneously as colophon and provenance (rare but legitimate). The infrastructure reuses the genre-classifier model class (\texttt{GenreClassificationModel}) with \texttt{num\_classes}~$=$~2 and the same focal-loss + per-class positive-weight scheme.

\textbf{Training data.} Distant supervision extracts positive labels from MARC structured fields: a sentence is COLOPHON-positive if it contains canonical Hebrew colophon-formula keywords (\texthebrew{נשלם}, \texthebrew{סיום}, \texthebrew{קולופון}) or is the closing sentence of MARC~500 in records that have a parallel MARC~561 entry; a sentence is PROVENANCE-positive if it contains a name that also appears in MARC~561 of the same record. Training uses 5-fold stratified cross-validation; each fold's classification thresholds are tuned independently per head by scanning $[0.2, 0.8]$ to maximise per-class F1. The threshold pair is stored in the checkpoint and applied at inference.

\textbf{Inference and integration.} At Stage~2, every MARC~500 sentence is scored. Sentences passing the COLOPHON threshold accumulate in \texttt{record["ml\_colophon\_sentences"]} and are appended to \texttt{record["colophon\_text"]}, where the existing P1684 mapper picks them up. Sentences passing the PROVENANCE threshold are routed through the Provenance NER pipeline (the same model that runs on MARC~561), and each emitted entity carries \texttt{from\_marc500: True} for downstream auditability. Both routings degrade gracefully when the classifier checkpoint is absent: the missing model triggers a no-op rather than a failure, and the pipeline runs as if MARC~500 were unclassified.

\textbf{Coverage impact.} Expected uplift on the 100-MS evaluation: P1684 (inscription) coverage from 41\% to approximately 55\%; P127 (owned by) and P11603 (transcribed by) from 17\% / 19\% to approximately 25\% / 25\%. The two-head design means the classifier amortises a single forward pass across both downstream lifts.
